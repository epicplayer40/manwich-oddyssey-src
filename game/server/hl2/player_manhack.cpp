...
BEGIN_DATADESC( CPlayer_Manhack )
	...
	DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputActivate),
	DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDeactivate),
END_DATADESC()
...
void CPlayer_Manhack::InputDeactivate( inputdata_t &inputdata )
{
	...
	CBaseEntity* pBlade = (CBaseEntity*)(pPlayer->GetActiveWeapon());
 	pPlayer->Weapon_Drop( GetActiveWeapon(),0,0 );
	if (m_pSaveWeapon)
	{
...
void CPlayer_Manhack::CheckBladeTrace(trace_t &tr)
{
	...
	{
		//fix
		//pHitEntity = CBaseEntity::Instance( pPlayer );
