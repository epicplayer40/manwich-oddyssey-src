...
void CWeaponCGuard::UpdateLasers( void )
{
	...
	for ( int i = 0; i < 4; i++ )
	{
		...
		UTIL_Beam( ofs, tr.endpos, m_beamIndex, 0, 0, 2.0f, 0.1f, 2, 0, 1, 0, 178, 0, 255, 32, 100 ); //255, 255, 255 default
		
		UTIL_Beam( ofs, tr.endpos, m_haloIndex, 0, 0, 2.0f, 0.1f, 4, 0, 1, 16, 150, 0, 255, 8, 100 ); //255, 255, 255 default
	}
}
...
void CWeaponCGuard::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
		case EVENT_WEAPON_SMG1:
		{
		m_flLastRangeTime = gpGlobals->curtime + 5.0f; //Don't fire for another 5 seconds
...
void CWeaponCGuard::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
	...
	CreateConcussiveBlast( tr.endpos, tr.plane.normal, this, 1.0 );
		...
		m_flLastRangeTime = gpGlobals->curtime + 5.0f;
