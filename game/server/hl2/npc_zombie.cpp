...
int random_zombie_model = 1;

#define DEFAULT_ZOMBIE_MODEL "models/zombie/classic.mdl"

ConVar	sk_zombie_health("sk_zombie_health", "0");
ConVar	sk_zombie_model("sk_zombie_model", DEFAULT_ZOMBIE_MODEL);
ConVar	sk_zombie_enable_model_override("sk_zombie_enable_model_override", "0");
...
void CZombie::Precache(void)
{
	...
	PrecacheModel("models/zombie/classic_2003.mdl");
...
void CZombie::Spawn(void)
{
	...
	random_zombie_model = (max - min + 1) * scaled + min;
...
void CZombie::SetZombieModel(void)
{
	...
	else
	{
		...
		if (sk_zombie_enable_model_override.GetBool()) //added this so that people must enable this to spawn zombies from the console or else the game crashes. If someone finds a better solution then tell me.
		{
		...
		{
			SetModel(sk_zombie_model.GetString());

			//			SetModelName(MAKE_STRING(pModelName));
			//			SetModelName((GetModelName()));
			//			SetModelName(AllocPooledString(pModelName));
		}
		//		else SetModelName(MAKE_STRING(DEFAULT_ZOMBIE_MODEL));
		SetModelName((GetModelName()));
		//		PrecacheModel( STRING( GetModelName() ) );
...
//-----------------------------------------------------------------------------
bool CZombie::IsChopped(const CTakeDamageInfo& info)
{
	const char* pModelName = STRING(GetModelName());

	if (!Q_stricmp(pModelName, "models/zombie/classic.mdl"))
	{
		return BaseClass::IsChopped(info);
	}
	else return false;
}
...
//-----------------------------------------------------------------------------
bool CZombie::CanBecomeRagdoll()
{
	/*
	if (IsCurSchedule(SCHED_DIE) && !m_fIsTorso)
		return true;
...
/*
bool CZombie::BecomeRagdollOnClient(const Vector &force)
{
	if (!CanBecomeRagdoll())
		return false;
	...
	// Become server-side ragdoll if we're flagged to do it
		...
		CBaseEntity *pRagdoll = CreateServerRagdoll(this, 0, info, COLLISION_GROUP_PASSABLE_DOOR);

		// Transfer our name to the new ragdoll
		pRagdoll->SetName(GetEntityName());
		pRagdoll->SetCollisionGroup(COLLISION_GROUP_PASSABLE_DOOR);
...
DECLARE_ANIMEVENT(AE_ZOMBIE_DIE)
