...
ConVar	  sk_antlion_worker_spit_grenade_radius		  ( "sk_antlion_worker_spit_grenade_radius","250", FCVAR_NONE, "Radius of effect for an antlion worker spit grenade.");
...
extern ConVar sk_strooper_spore_grenade_radius;

LINK_ENTITY_TO_CLASS( grenade_spit, CGrenadeSpit );
LINK_ENTITY_TO_CLASS( grenade_spit_hl2, CGrenadeSpit );
...
void CGrenadeSpit::GrenadeSpitTouch( CBaseEntity *pOther )
{
	...
	else
	{
		...
		UTIL_DecalTrace( pNewTrace, "DecalKleinerPuddle001a" );
	}
	...
	CTakeDamageInfo info( CTakeDamageInfo( this, GetThrower(), sk_antlion_worker_spit_grenade_dmg.GetFloat() * (1.0f-poisonratio), DMG_ACID | DMG_POISON ) );
	RadiusDamage( info, GetAbsOrigin(), sk_strooper_spore_grenade_radius.GetFloat(), CLASS_NONE, this );
