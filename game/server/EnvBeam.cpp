...
public:
	...
	void InputSetEndpoint( inputdata_t &inputdata );
...
BEGIN_DATADESC( CEnvBeam )
	...
	DEFINE_INPUTFUNC( FIELD_EHANDLE, "SetEndpoint", InputSetEndpoint ),
...
//-----------------------------------------------------------------------------
// Purpose: Input handler for setting the endpoint entity for this beam.
//-----------------------------------------------------------------------------
void CEnvBeam::InputSetEndpoint( inputdata_t &inputdata )
{
	if (inputdata.value.Entity().IsValid()) {
		SetEndEntity(inputdata.value.Entity().Get());
	}
}
