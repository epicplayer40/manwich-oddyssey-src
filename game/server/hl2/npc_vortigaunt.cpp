...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_Vortigaunt06 )
...
END_DATADESC()
LINK_ENTITY_TO_CLASS( npc_alien_slave, CNPC_Vortigaunt06 );
LINK_ENTITY_TO_CLASS( monster_alien_slave, CNPC_Vortigaunt06 );
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt06::IsHealPositionValid(void)
{
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt06::CheckHealPosition( void )
{
...
void CNPC_Vortigaunt06::SpeakSentence( int sentenceType )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask)
	{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//=========================================================
int CNPC_Vortigaunt06::GetSoundInterests ( void) 
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_CARCASS	|
...
// Output :
//-----------------------------------------------------------------------------
Class_T	CNPC_Vortigaunt06::Classify ( void )
{
...
//=========================================================
float CNPC_Vortigaunt06::MaxYawSpeed ( void )
{
	switch ( GetActivity() )
	{
	case ACT_IDLE:		
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_Vortigaunt06::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt06::MeleeAttack1Conditions ( float flDot, float flDist )
{
	if (flDist > 70)
	{
		return COND_TOO_FAR_TO_ATTACK;
...
// Output  :
//------------------------------------------------------------------------------
CBaseEntity*	CNPC_Vortigaunt06::Kick( void )
{
	trace_t tr;
...
//=========================================================
void CNPC_Vortigaunt06::HandleAnimEvent( animevent_t *pEvent )
{
...
// Output  :
//------------------------------------------------------------------------------
bool CNPC_Vortigaunt06::IsStompable(CBaseEntity *pEntity)
{
	if (!pEntity)
		return false;
...
// Output  :
//------------------------------------------------------------------------------
Activity CNPC_Vortigaunt06::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Output  :
//------------------------------------------------------------------------------
bool CNPC_Vortigaunt06::ShouldGoToIdleState( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::UpdateOnRemove( void)
{
	ClearBeams();
	ClearHandGlow();
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt06::Event_Killed( const CTakeDamageInfo &info )
{
	ClearBeams();
	ClearHandGlow();
...
bool CNPC_Vortigaunt06::CreateBehaviors()
{
...
// Init talk data
void CNPC_Vortigaunt06::TalkInit()
{
	BaseClass::TalkInit();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_Vortigaunt06::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
// PainSound
//=========================================================
void CNPC_Vortigaunt06::PainSound( const CTakeDamageInfo &info )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator* pAccumulator)
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt06::TranslateSchedule( int scheduleType )
{
	int baseType;
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt06::ShouldHealTarget( void )
{
	if ( IsInAScript() )
		return false;
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt06::SelectHealSchedule( void )
{
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_Vortigaunt06::SelectSchedule( void )
{
	ClearBeams();
	EndHandGlow();
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt06::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	if (interactionType == g_interactionBarnacleVictimDangle)
	{
		// Force choosing of a new schedule
		ClearSchedule("Dangling from barnacle");
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::DeclineFollowing( void )
{
	Speak( VORT_POK );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::PrescheduleThink( void )
{
	if ( m_bGlowTurningOn )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::BuildScheduleTestBits( void )
{
...
//=========================================================
void CNPC_Vortigaunt06::ArmBeam( int side, int beamType )
{
	trace_t tr;
	float flDist = 1.0;
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt06::DefendBeams( void )
{
	Vector vBeamStart;
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt06::ClawBeam( CBaseEntity *pHurt, int nNoise, int iAttachment )
{
	for (int i=0;i<2;i++)
	{
		CBeam* pBeam = CBeam::BeamCreate( "sprites/lgtning.vmt", 3.0 );
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt06::StartHandGlow( int beamType )
{
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt06::EndHandGlow( void )
{
	m_bGlowTurningOn	= false;
...
// Output :
//-----------------------------------------------------------------------------
Vector CNPC_Vortigaunt06::GetShootEnemyDir( const Vector &shootOrigin, bool bNoisy )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt06::IsValidEnemy( CBaseEntity *pEnemy )
{
	if ( IsRoller( pEnemy ) )
	{
...
//=========================================================
void CNPC_Vortigaunt06::BeamGlow( )
{
	int b = m_iBeams * 32;
	if (b > 255)
		b = 255;
...
//=========================================================
void CNPC_Vortigaunt06::WackBeam( int side, CBaseEntity *pEntity )
{
	Vector vecDest;
...
//=========================================================
void CNPC_Vortigaunt06::ZapBeam( int side )
{
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt06::HealBeam( int side )
{
	if (m_iBeams >= VORTIGAUNT_MAX_BEAMS)
		return;
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt06::ClearHandGlow( )
{
	if (m_pLeftHandGlow)
	{
...
//=========================================================
void CNPC_Vortigaunt06::ClearBeams( )
{
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::InputEnableArmorRecharge( inputdata_t &data )
{
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::InputDisableArmorRecharge( inputdata_t &data )
{
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::InputChargeTarget( inputdata_t &data )
{
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt06::InputExtractBugbait( inputdata_t &data )
{
	CBaseEntity *pTarget = gEntList.FindEntityByName( NULL, data.value.String(), NULL, data.pActivator, data.pCaller );
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt06::OnObstructionPreSteer( AILocalMoveGoal_t *pMoveGoal, float distClear, AIMoveResult_t *pResult )
{
...
AI_BEGIN_CUSTOM_NPC( npc_alien_slave, CNPC_Vortigaunt06 )
