...
const int MAX_PLAYER_SQUAD = 300;
...
int CNPC_Citizen::SelectSchedule()
{
	...
	if ( GetMoveType() == MOVETYPE_NONE && ( GetMoveParent() && FClassnameIs( GetMoveParent(), "func_tracktrain" ) ) )
	{
...
bool CNPC_Citizen::ShouldLookForBetterWeapon()
{
	if ( BaseClass::ShouldLookForBetterWeapon() )
	{
		...
		if( pWeapon )
		{
			...
			else if( FClassnameIs( pWeapon, "weapon_ar1" ) )
			{
				// Content to keep this weapon forever
				m_flNextWeaponSearchTime = OTHER_DEFER_SEARCH_TIME;
				bDefer = true;
			}
			else if( FClassnameIs( pWeapon, "weapon_hmg1" ) )
			{
				// Content to keep this weapon forever
				m_flNextWeaponSearchTime = OTHER_DEFER_SEARCH_TIME;
				bDefer = true;
