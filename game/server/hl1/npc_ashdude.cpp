...
//
// Purpose:		Ashdude from Opposing Force. Big scary-ass headcrab zombie, based heavily on the Bullsquid.
...
#define		ASHDUDE_SPRINT_DIST	256
...
ConVar sk_ashdude_health ( "sk_ashdude_health", "500" );
ConVar sk_ashdude_dmg_bite ( "sk_ashdude_dmg_bite", "25" );
ConVar sk_ashdude_dmg_whip ( "sk_ashdude_dmg_whip", "35" );
ConVar sk_ashdude_dmg_spit ( "sk_ashdude_dmg_spit", "15" );
...
enum
{
	SCHED_ASHDUDE_HURTHOP = LAST_SHARED_SCHEDULE,
	SCHED_ASHDUDE_SEECRAB,
	SCHED_ASHDUDE_EAT,
	SCHED_ASHDUDE_SNIFF_AND_EAT,
	SCHED_ASHDUDE_WALLOW,
	SCHED_ASHDUDE_CHASE_ENEMY,
};
...
enum 
{
	TASK_ASHDUDE_HOPTURN = LAST_SHARED_TASK,
	TASK_ASHDUDE_EAT,
};
...
enum
{
	COND_ASHDUDE_SMELL_FOOD	= LAST_SHARED_CONDITION + 1,
};
...
//=========================================================
#define		ASHDUDE_AE_SPIT		( 1 )
#define		ASHDUDE_AE_BITE		( 2 )
#define		ASHDUDE_AE_BLINK		( 3 )
#define		ASHDUDE_AE_TAILWHIP	( 4 )
#define		ASHDUDE_AE_HOP		( 5 )
#define		ASHDUDE_AE_THROW		( 6 )

LINK_ENTITY_TO_CLASS( monster_ashdude, CNPC_Ashdude );

int ACT_ASHDUDE_EXCITED;
int ACT_ASHDUDE_EAT;
int ACT_ASHDUDE_DETECT_SCENT;
int ACT_ASHDUDE_INSPECT_FLOOR;

//=========================================================
// Ashdude's spit projectile
//=========================================================
class CAshdudeSpit : public CBaseEntity
{
	DECLARE_CLASS( CAshdudeSpit, CBaseEntity );
public:
	void Spawn( void );
	void Precache( void );
...
void CAshdudeSpit::Precache( void )
{
	m_nAshdudeSpitSprite = PrecacheModel("sprites/ashdudespit.vmt");// client side spittle.
...
void CAshdudeSpit:: Spawn( void )
{
	...
	SetSprite( CSprite::SpriteCreate( "sprites/ashdudespit.vmt", GetAbsOrigin(), true ) );
...
void CAshdudeSpit::Shoot( CBaseEntity *pOwner, Vector vecStart, Vector vecVelocity )
{
	...
	te->SpriteSpray( filter, 0.0, &vecStart , &vecVelocity, pSpit->m_nAshdudeSpitSprite, 210, 25, 15 );
}

void CAshdudeSpit::Touch ( CBaseEntity *pOther )
{
	trace_t tr;
	int		iPitch;
	...
	if ( !pOther->m_takedamage )
	{
		...
		te->SpriteSpray( filter, 0.0,	&tr.endpos, &tr.plane.normal, m_nAshdudeSpitSprite, 30, 8, 5 );

	}
	else
	{
		CTakeDamageInfo info( this, this, sk_ashdude_dmg_spit.GetFloat(), DMG_BULLET );
...
// Spawn
//=========================================================
void CNPC_Ashdude::Spawn()
{
	Precache( );

	SetModel( "models/ashdude.mdl");
	...
	m_iHealth			= sk_ashdude_health.GetFloat();
...
void CNPC_Ashdude::Precache()
{
	...
	PrecacheModel("models/ashdude.mdl");
	
	PrecacheModel("sprites/ashdudespit.vmt");// spit projectile.

	PrecacheScriptSound( "Ashdude.Idle" );
	PrecacheScriptSound( "Ashdude.Pain" );
	PrecacheScriptSound( "Ashdude.Alert" );
	PrecacheScriptSound( "Ashdude.Die" );
	PrecacheScriptSound( "Ashdude.Attack" );
	PrecacheScriptSound( "Ashdude.Bite" );
	PrecacheScriptSound( "Ashdude.Growl" );
	PrecacheScriptSound( "Ashdude.Shoot" );
}


int CNPC_Ashdude::TranslateSchedule( int scheduleType )
{	
	switch	( scheduleType )
	{
		case SCHED_CHASE_ENEMY:
			return SCHED_ASHDUDE_CHASE_ENEMY;
			break;
	}
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CNPC_Ashdude::Classify( void )
{
	return CLASS_CONSCRIPT; 
}

bool CNPC_Ashdude::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
...
// IdleSound 
//=========================================================
#define ASHDUDE_ATTN_IDLE	(float)1.5
void CNPC_Ashdude::IdleSound( void )
{
	CPASAttenuationFilter filter( this, ASHDUDE_ATTN_IDLE );
	EmitSound( filter, entindex(), "Ashdude.Idle" );	
}

//=========================================================
// PainSound 
//=========================================================
void CNPC_Ashdude::PainSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ashdude.Pain" );	
}

//=========================================================
// AlertSound
//=========================================================
void CNPC_Ashdude::AlertSound( void )
{
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ashdude.Alert" );	
}

//=========================================================
// DeathSound
//=========================================================
void CNPC_Ashdude::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ashdude.Die" );	
}

//=========================================================
// AttackSound
//=========================================================
void CNPC_Ashdude::AttackSound( void )
{
	CPASAttenuationFilter filter( this );
	EmitSound( filter, entindex(), "Ashdude.Attack" );	
}
...
//=========================================================
float CNPC_Ashdude::MaxYawSpeed( void )
{
	float flYS = 0;
...
bool CNPC_Ashdude::ShouldIgnite( const CTakeDamageInfo &info )
{
 	if ( IsOnFire() )
	{
		// Already burning!
...
void CNPC_Ashdude::PrescheduleThink( void )
{
...
//=========================================================
void CNPC_Ashdude::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
		case ASHDUDE_AE_SPIT:
		{
			if ( GetEnemy() )
			{
...
//	FireBullets(7, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 1024, m_iAmmoType );
	...
	FireBullets(16, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 8096, m_iAmmoType );
	...
	CSoundEnt::InsertSound ( SOUND_COMBAT, GetAbsOrigin(), 384, 0.3 );
		...
		case ASHDUDE_AE_BITE:
		{
		// SOUND HERE!
			CPASAttenuationFilter filter( this );
			//CBaseEntity *pHurt = CheckTraceHullAttack( 70, Vector(-16,-16,-16), Vector(16,16,16), sk_ashdude_dmg_bite.GetFloat(), DMG_SLASH );
			Vector vecMins = GetHullMins();
			Vector vecMaxs = GetHullMaxs();
			vecMins.z = vecMins.x;
			vecMaxs.z = vecMaxs.x;

			CBaseEntity *pHurt = CheckTraceHullAttack( 70, vecMins, vecMaxs, sk_ashdude_dmg_bite.GetFloat(), DMG_SLASH );
			if ( pHurt )
			{
				Vector forward, up;
				...
				EmitSound( filter, entindex(), "Ashdude.Bite" );
			}
		...
		case ASHDUDE_AE_TAILWHIP:
		{
			...
			CPASAttenuationFilter filter( this );
			EmitSound( filter, entindex(), "Ashdude.Bite" );	
			//CBaseEntity *pHurt = CheckTraceHullAttack( 70, Vector(-16,-16,-16), Vector(16,16,16), sk_ashdude_dmg_whip.GetFloat(), DMG_SLASH | DMG_ALWAYSGIB );
				CBaseEntity *pHurt = CheckTraceHullAttack( 70, vecMins, vecMaxs, sk_ashdude_dmg_whip.GetFloat(), DMG_SLASH | DMG_ALWAYSGIB );
			if ( pHurt ) 
			{
				Vector right, up;
...
int CNPC_Ashdude::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	if ( flDist > 85 && flDist <= 784 && flDot >= 0.5 && gpGlobals->curtime >= m_flNextSpitTime )
	{
		...
		if ( IsMoving() )
		{
			...
			m_flNextSpitTime = gpGlobals->curtime + RandomInt(1, 5);
		}
		else
		{
...
//=========================================================
// MeleeAttack2Conditions - ashdude is a big guy, so has a longer
// melee range than most monsters. This is the tailwhip attack
//=========================================================
int CNPC_Ashdude::MeleeAttack1Conditions( float flDot, float flDist )
{
	if ( GetEnemy()->m_iHealth <= sk_ashdude_dmg_whip.GetFloat() && flDist <= 85 && flDot >= 0.7 )
	{
...
// is valid.
//=========================================================
int CNPC_Ashdude::MeleeAttack2Conditions( float flDot, float flDist )
{
...
bool CNPC_Ashdude::FValidateHintType ( CAI_Hint *pHint )
{
...
void CNPC_Ashdude::RemoveIgnoredConditions( void )
{
	if ( m_flHungryTime > gpGlobals->curtime )
		 ClearCondition( COND_ASHDUDE_SMELL_FOOD );
...
Disposition_t CNPC_Ashdude::IRelationType( CBaseEntity *pTarget )
{
...
//=========================================================
// TakeDamage - overridden for ashdude so we can keep track
// of how much time has passed since it was last injured
//=========================================================
int CNPC_Ashdude::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
//=========================================================
int CNPC_Ashdude::GetSoundInterests ( void )
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
		    SOUND_CARCASS	|
...
//=========================================================
void CNPC_Ashdude::OnListened( void )
{
	AISoundIter_t iter;
	...
	while ( pCurrentSound )
	{
		...
		if ( !pCurrentSound->FIsSound() )
		{
			...
			if ( pCurrentSound->IsSoundType( SOUND_MEAT | SOUND_CARCASS ) )
			{
				...
				condition = COND_ASHDUDE_SMELL_FOOD;
			}
		}
...
//========================================================
// RunAI - overridden for ashdude because there are things
// that need to be checked every think.
//========================================================
void CNPC_Ashdude::RunAI ( void )
{
	...
	if ( GetEnemy() != NULL && GetActivity() == ACT_RUN )
	{
		...
		if ( (GetAbsOrigin() - GetEnemy()->GetAbsOrigin()).Length2D() < ASHDUDE_SPRINT_DIST )
		{
...
void CNPC_Ashdude::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
...
// GetSchedule 
//=========================================================
int CNPC_Ashdude::SelectSchedule( void )
{
	switch	( m_NPCState )
	{
	case NPC_STATE_ALERT:
		{
			...
			{
				return SCHED_ASHDUDE_HURTHOP;
			}

			if ( HasCondition( COND_ASHDUDE_SMELL_FOOD ) )
			{
				CSound		*pSound;
				...
				if ( pSound && (!FInViewCone ( pSound->GetSoundOrigin() ) || !FVisible ( pSound->GetSoundOrigin() )) )
				{
					...
					return SCHED_ASHDUDE_SNIFF_AND_EAT;
				}

				// food is right out in the open. Just go get it.
				return SCHED_ASHDUDE_EAT;
			}
...
//=========================================================
bool CNPC_Ashdude::FInViewCone ( Vector pOrigin )
{
...
//=========================================================
bool CNPC_Ashdude::FVisible ( Vector vecOrigin )
{
	trace_t tr;
...
// schedule.  OVERRIDDEN for ashdude because it needs to
// know explicitly when the last attempt to chase the enemy
// failed, since that impacts its attack choices.
//=========================================================
void CNPC_Ashdude::StartTask ( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_MELEE_ATTACK2:
		{
			CPASAttenuationFilter filter( this );
			EmitSound( filter, entindex(), "Ashdude.Growl" );		
	...
	case TASK_RANGE_ATTACK1:
		{
			CPASAttenuationFilter filter( this );
			EmitSound( filter, entindex(), "Ashdude.Shoot" );		
			BaseClass::StartTask ( pTask );
			break;
		}
	case TASK_ASHDUDE_HOPTURN:
		{
			SetActivity ( ACT_HOP );
...
// RunTask
//=========================================================
void CNPC_Ashdude::RunTask ( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_ASHDUDE_HOPTURN:
...
//=========================================================
// GetIdealState - Overridden for Ashdude to deal with
// the feature that makes it lose interest in headcrabs for 
// a while if something injures it. 
//=========================================================
NPC_STATE CNPC_Ashdude::SelectIdealState ( void )
{
...
AI_BEGIN_CUSTOM_NPC( monster_ashdude, CNPC_Ashdude )

	DECLARE_TASK ( TASK_ASHDUDE_HOPTURN )
	DECLARE_TASK ( TASK_ASHDUDE_EAT )

	DECLARE_CONDITION( COND_ASHDUDE_SMELL_FOOD )

	DECLARE_ACTIVITY( ACT_ASHDUDE_EXCITED )
	DECLARE_ACTIVITY( ACT_ASHDUDE_EAT )
	DECLARE_ACTIVITY( ACT_ASHDUDE_DETECT_SCENT )
	DECLARE_ACTIVITY( ACT_ASHDUDE_INSPECT_FLOOR )

	//=========================================================
	// > SCHED_ASHDUDE_HURTHOP
	//=========================================================
	DEFINE_SCHEDULE
	(
		SCHED_ASHDUDE_HURTHOP,
	...
	(
		...
		"		TASK_STOP_MOVING			0"
		"		TASK_SOUND_WAKE				0"
		"		TASK_PLAY_SEQUENCE			ACTIVITY:ACT_ASHDUDE_EXCITED"
		"		TASK_FACE_ENEMY				0"
		"	"
		"	Interrupts"
		"		COND_LIGHT_DAMAGE"
	...
	(
		...
		"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_ASHDUDE_EAT"
		"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_ASHDUDE_EAT"
		"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_ASHDUDE_EAT"
		"		TASK_ASHDUDE_EAT						50"
	...
	(
		...
		"		TASK_STOP_MOVING					0"
		"		TASK_ASHDUDE_EAT						10"
		"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_ASHDUDE_DETECT_SCENT"
		"		TASK_STORE_LASTPOSITION				0"
		"		TASK_GET_PATH_TO_BESTSCENT			0"
		"		TASK_WALK_PATH						0"
		"		TASK_WAIT_FOR_MOVEMENT				0"
	...
	(
		...
		"		TASK_WALK_PATH					0"
		"		TASK_WAIT_FOR_MOVEMENT			0"
		"		TASK_PLAY_SEQUENCE				ACTIVITY:ACT_ASHDUDE_INSPECT_FLOOR"
		"		TASK_ASHDUDE_EAT					50"
		"		TASK_GET_PATH_TO_LASTPOSITION	0"
		"		TASK_WALK_PATH					0"
		"		TASK_WAIT_FOR_MOVEMENT			0"
