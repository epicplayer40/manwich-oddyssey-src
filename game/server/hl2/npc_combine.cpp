...
string_t	s_iszDbarrelClassname;
//-----------------------------------------------------------------------------
// Interactions
//-----------------------------------------------------------------------------
...
void CNPC_Combine::Activate()
{
	...
	s_iszDbarrelClassname = FindPooledString( "weapon_dbarrel" );
	BaseClass::Activate();
}
	...
	case SCHED_HIDE_AND_RELOAD:
		{
			...
			if ( IsMossman() && Weapon_OwnsThisType( "weapon_dbarrel" ) )
			{
			...
			if ( IsMossman() && Weapon_OwnsThisType( "weapon_shotgun" ) )
			{
				if (GetActiveWeapon()->UsesClipsForAmmo1())
				{
				  GetActiveWeapon()->m_iClip1 = GetActiveWeapon()->GetMaxClip1();
...
void CNPC_Combine::PainSound ( const CTakeDamageInfo &info )
{
	if ( gpGlobals->curtime > m_flNextPainSoundTime )
	{
		...
		if ( random->RandomInt(0,99) < 20 )
		{
			// pain sentences are rare
			if (FOkToMakeSound())
			{
				m_Sentences.Speak( "COMBINE_TAUNT", SENTENCE_PRIORITY_HIGH );
				JustMadeSound();
				return;
			}
		}

		m_Sentences.Speak( "COMBINE_PAIN", SENTENCE_PRIORITY_HIGH );
...
WeaponProficiency_t CNPC_Combine::CalcWeaponProficiency( CBaseCombatWeapon *pWeapon )
{
	...
	else if ( FClassnameIs( pWeapon, "weapon_dbarrel" ) )
	{
		if( m_nSkin != COMBINE_SKIN_SHOTGUNNER )
		{
			m_nSkin = COMBINE_SKIN_SHOTGUNNER;
	...
	else if( FClassnameIs( pWeapon, "weapon_tommygun" ) )
	{
		return WEAPON_PROFICIENCY_VERY_GOOD;
	}
...
bool CNPC_Combine::HasDbarrel()
{
	if( GetActiveWeapon() && GetActiveWeapon()->m_iClassname == s_iszDbarrelClassname )
	{
		return true;
	}
...
(
 ...
 DEFINE_SCHEDULE
(
	SCHED_COMBINE_ESTABLISH_RPG_LINE_OF_FIRE,
	...
	"		TASK_RUN_PATH					0"
	"		TASK_WAIT_FOR_MOVEMENT			0"
	"		TASK_SET_SCHEDULE				SCHEDULE:SCHED_COMBINE_FIRE_RPG"
	""
	"	Interrupts "
	"		COND_NEW_ENEMY"
	"		COND_ENEMY_DEAD"
 ...
 DEFINE_SCHEDULE
(
	SCHED_COMBINE_FIRE_RPG,

	"	Tasks"
	"		TASK_STOP_MOVING			0"
	"		TASK_FACE_ENEMY				0"
	"		TASK_ANNOUNCE_ATTACK		1"	// 1 = primary attack
	"		TASK_WAIT_RANDOM			1.50"
