...
ConVar sk_combine_mossman_health( "sk_combine_mossman_health", "40");
ConVar sk_TsoLing_health( "sk_TsoLing_health", "750");
...
void CNPC_CombineS::Spawn( void )
{
	...
	{
		// Stronger, tougher.
		SetHealth( sk_combine_mossman_health.GetFloat() );
		SetMaxHealth( sk_combine_mossman_health.GetFloat() );
		SetKickDamage( sk_combine_guard_kick.GetFloat() );
	}
	if( IsTsoLing() )
	{
		// Stronger, tougher.
		SetHealth( sk_TsoLing_health.GetFloat() );
		SetMaxHealth( sk_TsoLing_health.GetFloat() );
		SetKickDamage( sk_combine_guard_kick.GetFloat() );
	}
...
void CNPC_CombineS::Precache()
{
	...
	if( !Q_stricmp( pModelName, "models/mossman.mdl" ) )
	{
		m_fIsMossman = true;
	}
	else
	...
	if( !Q_stricmp( pModelName, "models/General_Ling.mdl" ) )
	{
		m_fIsTsoLing = true;
	}
	else
...
int CNPC_CombineS::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if( IsTsoLing() )
	{
		if ( !info.GetAttacker()->IsPlayer() )
		{
			...
			DevMsg( "Tso-Ling Shot by NPC, only 15 percent damage taken\n" );
			}	
...
Activity CNPC_CombineS::NPC_TranslateActivity( Activity eNewActivity )
{
	...
	{
		eNewActivity = ACT_RANGE_ATTACK_SMG1;
	}
