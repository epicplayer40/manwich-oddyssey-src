...
//4: Make a version with its own CLASS_ faction for each of the 5 variants for after the militia breaks off, monster_alphagarney_team1 with CLASS_TEAM1, monster_alphagarney_team2 with CLASS_TEAM2, etc. (Haven't started yet, it'll be tedious)
...
#include	"hl1_green_Alphabarney.h"
...
ConVar	sk_alphagarneyhl1_health( "sk_alphagarneyhl1_health","100");

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
// first flag is alphagarney dying for scripted sequences?
#define		ALPHAGARNEY_AE_DRAW		( 2 )
#define		ALPHAGARNEY_AE_SHOOT		( 3 )
#define		ALPHAGARNEY_AE_SHOTGUN		( 5 )
#define		ALPHAGARNEY_AE_SNIPER		( 6 )
#define		ALPHAGARNEY_AE_SMG			( 7 )
#define		ALPHAGARNEY_AE_GRENADETHROW		( 8 )
#define		ALPHAGARNEY_AE_HOLSTER	( 4 )

#define		ALPHAGARNEY_BODY_GUNHOLSTERED	0
#define		ALPHAGARNEY_BODY_GUNDRAWN		1
#define		ALPHAGARNEY_BODY_GUNGONE			2
#define		ALPHAGARNEY_BODY_SHOTGUN		3
#define		ALPHAGARNEY_BODY_SMG		4
#define		ALPHAGARNEY_BODY_SNIPER	5

#define GARNEY_PISTOL			( 1 << 0)
#define GARNEY_SHOTGUN			( 1 << 1)
#define GARNEY_SNIPER			( 1 << 2)
#define GARNEY_SMG				( 1 << 3)


enum AlphaGarneySquadSlot_T
{	
	ALPHAGARNEY_SQUAD_SLOT_ATTACK1 = LAST_SHARED_SQUADSLOT,
	ALPHAGARNEY_SQUAD_SLOT_ATTACK2,
	ALPHAGARNEY_SQUAD_SLOT_INVESTIGATE_SOUND,
	ALPHAGARNEY_SQUAD_SLOT_CHASE,
};
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_HL1AlphaGarney )
...
LINK_ENTITY_TO_CLASS( monster_alphabarney_green, CNPC_HL1AlphaGarney );
...
// Spawn
//=========================================================
void CNPC_HL1AlphaGarney::Spawn()
{
	Precache( );
	...
	m_iHealth			= sk_alphagarneyhl1_health.GetFloat();
	...
	//	m_iWeapons = TRESH_HANDGRENADE;
		...
		{
		case 1:
			m_iWeapons = GARNEY_PISTOL;
			break;
		case 2:
			m_iWeapons = GARNEY_SMG;
			break;
		case 3:
			m_iWeapons = GARNEY_SHOTGUN;
			break;
		case 4:
			m_iWeapons = GARNEY_SNIPER;
			break;
		}
	}

	if (FBitSet(m_iWeapons, GARNEY_PISTOL))
	{
		SetBodygroup( 1, ALPHAGARNEY_BODY_GUNDRAWN	);
	}

	if (FBitSet(m_iWeapons, GARNEY_SMG))
	{
		SetBodygroup( 1, ALPHAGARNEY_BODY_SMG	);
	}

	if (FBitSet(m_iWeapons, GARNEY_SHOTGUN))
	{
		SetBodygroup( 1, ALPHAGARNEY_BODY_SHOTGUN	);
	}
	if (FBitSet(m_iWeapons, GARNEY_SNIPER))
	{
		SetBodygroup( 1, ALPHAGARNEY_BODY_SNIPER	);
	}

	
	NPCInit();
	
	SetUse( &CNPC_HL1AlphaGarney::FollowerUse );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CNPC_HL1AlphaGarney::Precache()
{
	...
	// every new alphagarney must call this, otherwise
	// when a level is loaded, nobody will talk (time is reset to 0)
	TalkInit();
	BaseClass::Precache();
}	

void CNPC_HL1AlphaGarney::ModifyOrAppendCriteria( AI_CriteriaSet& criteriaSet )
{
...
// Init talk data
void CNPC_HL1AlphaGarney::TalkInit()
{
	BaseClass::TalkInit();

	// get voice for head - just one alphagarney voice for now
...
//=========================================================
int CNPC_HL1AlphaGarney::GetSoundInterests ( void) 
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_CARCASS	|
...
// relationship table.
//=========================================================
Class_T	CNPC_HL1AlphaGarney::Classify ( void )
{
	return	CLASS_CONSCRIPT;
}

//=========================================================
// ALertSound - alphagarney says "Freeze!"
//=========================================================
void CNPC_HL1AlphaGarney::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
	//	if ( IsOkToSpeak() )
...
//=========================================================
void CNPC_HL1AlphaGarney::SetYawSpeed ( void )
{
	int ys;
...
// CheckRangeAttack1
//=========================================================
bool CNPC_HL1AlphaGarney::CheckRangeAttack1 ( float flDot, float flDist )
{
...
int CNPC_HL1AlphaGarney::RangeAttack2Conditions(float flDot, float flDist)
{
	m_iLastGrenadeCondition = GetGrenadeConditions(flDot, flDist);
	return m_iLastGrenadeCondition;
}

int CNPC_HL1AlphaGarney::GetGrenadeConditions(float flDot, float flDist)
{
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_HL1AlphaGarney::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
//=========================================================
// AlphaGarneyFirePistol - shoots one round from the pistol at
// the enemy alphagarney is facing.
//=========================================================
void CNPC_HL1AlphaGarney::AlphaGarneyFirePistol ( void )
{
...
void CNPC_HL1AlphaGarney::AlphaGarneyFireShotgun ( void )
{
	Vector vecShootOrigin;
...
void CNPC_HL1AlphaGarney::AlphaGarneyFireSMG( void )
{
	Vector vecShootOrigin;
...
void CNPC_HL1AlphaGarney::AlphaGarneyFireSniper ( void )
{
	Vector vecShootOrigin;
...
int CNPC_HL1AlphaGarney::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
// PainSound
//=========================================================
void CNPC_HL1AlphaGarney::PainSound( const CTakeDamageInfo &info )
{
...
// DeathSound 
//=========================================================
void CNPC_HL1AlphaGarney::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );
...
void CNPC_HL1AlphaGarney::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_HL1AlphaGarney::Event_Killed( const CTakeDamageInfo &info )
{
	if ( m_nBody < ALPHAGARNEY_BODY_GUNGONE )
	{
		// drop the gun!
		Vector vecGunPos;
		QAngle angGunAngles;
		...
		SetBodygroup( 1, ALPHAGARNEY_BODY_GUNGONE);
...
void CNPC_HL1AlphaGarney::SUB_StartLVFadeOut( float delay, bool notSolid )
{
	SetThink( &CNPC_HL1AlphaGarney::SUB_LVFadeOut );
...
void CNPC_HL1AlphaGarney::SUB_LVFadeOut( void  )
{
...
void CNPC_HL1AlphaGarney::StartTask( const Task_t *pTask )
{
	BaseClass::StartTask( pTask );	
}

void CNPC_HL1AlphaGarney::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//=========================================================
void CNPC_HL1AlphaGarney::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
	case ALPHAGARNEY_AE_SHOOT:
		AlphaGarneyFirePistol();
		if (FBitSet(m_iWeapons, GARNEY_PISTOL))
		{
			AlphaGarneyFirePistol();
		}
		...
		if (FBitSet(m_iWeapons, GARNEY_SNIPER))
		{
			AlphaGarneyFireSniper();
		}
		break;

		case ALPHAGARNEY_AE_GRENADETHROW:
		{
...
/*	case ALPHAGARNEY_AE_SHOTGUN:
		AlphaGarneyFireShotgun();
		break;
	case ALPHAGARNEY_AE_SNIPER:
		AlphaGarneyFireSniper();
		break;
	case ALPHAGARNEY_AE_SMG:
		AlphaGarneyFireSMG();
		break;
*/ //don't think these are necessary
	case ALPHAGARNEY_AE_DRAW:
		// alphagarney's bodygroup switches here so he can pull gun from holster
		if (FBitSet(m_iWeapons, GARNEY_PISTOL))
		{
			SetBodygroup( 1, ALPHAGARNEY_BODY_GUNDRAWN);
		}
	...
	case ALPHAGARNEY_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		SetBodygroup( 1, ALPHAGARNEY_BODY_GUNHOLSTERED);
		m_fGunDrawn = false;
		break;
...
int CNPC_HL1AlphaGarney::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	case SCHED_ARM_WEAPON:
		if ( GetEnemy() != NULL )
		{
			...
			return SCHED_ALPHAGARNEY_ENEMY_DRAW;
		}
		break;
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_ALPHAGARNEY_FACE_TARGET;
			else
				return baseType;
		}
		break;
	...
	{
		return SCHED_ALPHAGARNEY_FOLLOW;
		break;
	}
	...
	case SCHED_IDLE_STAND:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_ALPHAGARNEY_IDLE_STAND;
			else
				return baseType;
		}
		break;
...
// of the proper type.
//=========================================================
int CNPC_HL1AlphaGarney::SelectSchedule( void )
{
...
NPC_STATE CNPC_HL1AlphaGarney::SelectIdealState ( void )
{
	return BaseClass::SelectIdealState();
}

void CNPC_HL1AlphaGarney::DeclineFollowing( void )
{
...
bool CNPC_HL1AlphaGarney::CanBecomeRagdoll( void )
{
	if ( UTIL_IsLowViolence() )
	{
		return false;
...
bool CNPC_HL1AlphaGarney::ShouldGib( const CTakeDamageInfo &info )
{
//	if ( UTIL_IsLowViolence() )
//	{
//		return false;
...
bool CNPC_HL1AlphaGarney::CorpseGib( const CTakeDamageInfo &info )
{
	CEffectData	data;
...
AI_BEGIN_CUSTOM_NPC( monster_alphagarney, CNPC_HL1AlphaGarney )

	DECLARE_SQUADSLOT( ALPHAGARNEY_SQUAD_SLOT_ATTACK1 )
	DECLARE_SQUADSLOT( ALPHAGARNEY_SQUAD_SLOT_ATTACK2 )
	DECLARE_SQUADSLOT( ALPHAGARNEY_SQUAD_SLOT_CHASE )
	DECLARE_SQUADSLOT( ALPHAGARNEY_SQUAD_SLOT_INVESTIGATE_SOUND )

	//=========================================================
	// > SCHED_ALPHAGARNEY_FOLLOW
	//=========================================================
	DEFINE_SCHEDULE
	(
		SCHED_ALPHAGARNEY_FOLLOW,
		...
		"	Tasks"
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_ALPHAGARNEY_STOP_FOLLOWING"
	...
	(
		...
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_ALPHAGARNEY_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
...
//
//=========================================================
class CNPC_DeadAlphaGarney : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_DeadAlphaGarney, CAI_BaseNPC );
public:
...
char *CNPC_DeadAlphaGarney::m_szPoses[] = { "lying_on_back", "lying_on_side", "lying_on_stomach" };

bool CNPC_DeadAlphaGarney::KeyValue( const char *szKeyName, const char *szValue )
{
...
LINK_ENTITY_TO_CLASS( monster_alphagarney_dead, CNPC_DeadAlphaGarney );

BEGIN_DATADESC( CNPC_DeadAlphaGarney )
END_DATADESC()

//=========================================================
// ********** DeadAlphaGarney SPAWN **********
//=========================================================
void CNPC_DeadAlphaGarney::Spawn( void )
{
	PrecacheModel("models/alphagarneyhl1.mdl");
	SetModel( "models/alphagarneyhl1.mdl");
	...
	if ( GetSequence() == -1 )
	{
		Msg ( "Dead alphagarney with bad pose\n" );
	}
	// Corpses have less health
	m_iHealth			= 0.0;//gSkillData.alphagarneyHealth;
