...
ConVar sk_controller_health ( "sk_controller_health", "120" );
...
void CNPC_Controller::Precache()
{
	...
	PrecacheModel( "sprites/blueflare.vmt");
	...
	PrecacheScriptSound( "Controller.ElectroSound" );
...
//	if ( HasMemory( bits_MEMORY_KILLED ) )
	...
	for (int i = 0; i < 2; i++)
	{
		if (m_pBall[i] == NULL)
		{
			m_pBall[i] = CSprite::SpriteCreate( "sprites/blueflare.vmt", GetAbsOrigin(), TRUE );
...
void CNPC_ControllerHeadBall::Spawn( void )
{
	...
	m_pSprite = CSprite::SpriteCreate( "sprites/blueflare.vmt", GetAbsOrigin(), FALSE );
