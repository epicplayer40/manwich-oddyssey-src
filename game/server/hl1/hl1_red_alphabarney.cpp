...
//4: Make a version with its own CLASS_ faction for each of the 5 variants for after the militia breaks off, monster_alphararney_team1 with CLASS_TEAM1, monster_alphararney_team2 with CLASS_TEAM2, etc. (Haven't started yet, it'll be tedious)
...
ConVar	sk_alphararneyhl1_health( "sk_alphararneyhl1_health","100");

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
// first flag is alphararney dying for scripted sequences?
#define		ALPHARARNEY_AE_DRAW		( 2 )
#define		ALPHARARNEY_AE_SHOOT		( 3 )
#define		ALPHARARNEY_AE_SHOTGUN		( 5 )
#define		ALPHARARNEY_AE_SNIPER		( 6 )
#define		ALPHARARNEY_AE_SMG			( 7 )
#define		ALPHARARNEY_AE_GRENADETHROW		( 8 )
#define		ALPHARARNEY_AE_HOLSTER	( 4 )

#define		ALPHARARNEY_BODY_GUNHOLSTERED	0
#define		ALPHARARNEY_BODY_GUNDRAWN		1
#define		ALPHARARNEY_BODY_GUNGONE			2
#define		ALPHARARNEY_BODY_SHOTGUN		3
#define		ALPHARARNEY_BODY_SMG		4
#define		ALPHARARNEY_BODY_SNIPER	5

#define RARNEY_PISTOL			( 1 << 0)
#define RARNEY_SHOTGUN			( 1 << 1)
#define RARNEY_SNIPER			( 1 << 2)
#define RARNEY_SMG				( 1 << 3)


enum AlphaRarneySquadSlot_T
{	
	ALPHARARNEY_SQUAD_SLOT_ATTACK1 = LAST_SHARED_SQUADSLOT,
	ALPHARARNEY_SQUAD_SLOT_ATTACK2,
	ALPHARARNEY_SQUAD_SLOT_INVESTIGATE_SOUND,
	ALPHARARNEY_SQUAD_SLOT_CHASE,
};
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_HL1AlphaRarney )
...
LINK_ENTITY_TO_CLASS( monster_alphabarney_red, CNPC_HL1AlphaRarney );
...
// Spawn
//=========================================================
void CNPC_HL1AlphaRarney::Spawn()
{
	Precache( );
	...
	m_iHealth			= sk_alphararneyhl1_health.GetFloat();
	...
	{
		SetBodygroup( 1, ALPHARARNEY_BODY_GUNDRAWN	);
	}

	if (FBitSet(m_iWeapons, RARNEY_SMG))
	{
		SetBodygroup( 1, ALPHARARNEY_BODY_SMG	);
	}

	if (FBitSet(m_iWeapons, RARNEY_SHOTGUN))
	{
		SetBodygroup( 1, ALPHARARNEY_BODY_SHOTGUN	);
	}
	if (FBitSet(m_iWeapons, RARNEY_SNIPER))
	{
		SetBodygroup( 1, ALPHARARNEY_BODY_SNIPER	);
	}

	
	NPCInit();
	
	SetUse( &CNPC_HL1AlphaRarney::FollowerUse );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CNPC_HL1AlphaRarney::Precache()
{
	if (FBitSet(m_iWeapons, RARNEY_SNIPER))
	{
	...
	PrecacheModel( "models/barnred.mdl" );

	PrecacheScriptSound( "AlphaBlarney.FirePistol" );
	PrecacheScriptSound( "AlphaBlarney.Pain" );
	PrecacheScriptSound( "AlphaBlarney.Die" );

	// every new alphararney must call this, otherwise
	// when a level is loaded, nobody will talk (time is reset to 0)
	TalkInit();
	BaseClass::Precache();
}	

void CNPC_HL1AlphaRarney::ModifyOrAppendCriteria( AI_CriteriaSet& criteriaSet )
{
...
// Init talk data
void CNPC_HL1AlphaRarney::TalkInit()
{
	BaseClass::TalkInit();

	// get voice for head - just one alphararney voice for now
...
//=========================================================
int CNPC_HL1AlphaRarney::GetSoundInterests ( void) 
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_CARCASS	|
...
// relationship table.
//=========================================================
Class_T	CNPC_HL1AlphaRarney::Classify ( void )
{
	return	CLASS_TEAM1;
}

//=========================================================
// ALertSound - alphararney says "Freeze!"
//=========================================================
void CNPC_HL1AlphaRarney::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
	//	if ( IsOkToSpeak() )
...
//=========================================================
void CNPC_HL1AlphaRarney::SetYawSpeed ( void )
{
	int ys;
...
// CheckRangeAttack1
//=========================================================
bool CNPC_HL1AlphaRarney::CheckRangeAttack1 ( float flDot, float flDist )
{
...
int CNPC_HL1AlphaRarney::RangeAttack2Conditions(float flDot, float flDist)
{
	m_iLastGrenadeCondition = GetGrenadeConditions(flDot, flDist);
	return m_iLastGrenadeCondition;
}

int CNPC_HL1AlphaRarney::GetGrenadeConditions(float flDot, float flDist)
{
	...
	if ((vecTarget - GetAbsOrigin()).Length2D() <= 256)
	{
		...
		if (vecToss != vec3_origin)
		{
			...
			m_flNextGrenadeCheck = gpGlobals->curtime + 10;
		...
		{
			// don't check again for a while.
			m_flNextGrenadeCheck = gpGlobals->curtime + 20;
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_HL1AlphaRarney::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
//=========================================================
// AlphaRarneyFirePistol - shoots one round from the pistol at
// the enemy alphararney is facing.
//=========================================================
void CNPC_HL1AlphaRarney::AlphaRarneyFirePistol ( void )
{
...
void CNPC_HL1AlphaRarney::AlphaRarneyFireShotgun ( void )
{
	Vector vecShootOrigin;
...
void CNPC_HL1AlphaRarney::AlphaRarneyFireSMG( void )
{
	Vector vecShootOrigin;
...
void CNPC_HL1AlphaRarney::AlphaRarneyFireSniper ( void )
{
	Vector vecShootOrigin;
...
int CNPC_HL1AlphaRarney::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
// PainSound
//=========================================================
void CNPC_HL1AlphaRarney::PainSound( const CTakeDamageInfo &info )
{
...
// DeathSound 
//=========================================================
void CNPC_HL1AlphaRarney::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );
...
void CNPC_HL1AlphaRarney::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_HL1AlphaRarney::Event_Killed( const CTakeDamageInfo &info )
{
	if ( m_nBody < ALPHARARNEY_BODY_GUNGONE )
	{
		// drop the gun!
		Vector vecGunPos;
		QAngle angGunAngles;
		...
		SetBodygroup( 1, ALPHARARNEY_BODY_GUNGONE);
...
void CNPC_HL1AlphaRarney::SUB_StartLVFadeOut( float delay, bool notSolid )
{
	SetThink( &CNPC_HL1AlphaRarney::SUB_LVFadeOut );
...
void CNPC_HL1AlphaRarney::SUB_LVFadeOut( void  )
{
...
void CNPC_HL1AlphaRarney::StartTask( const Task_t *pTask )
{
	BaseClass::StartTask( pTask );	
}

void CNPC_HL1AlphaRarney::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//=========================================================
void CNPC_HL1AlphaRarney::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
	case ALPHARARNEY_AE_SHOOT:
		AlphaRarneyFirePistol();
		if (FBitSet(m_iWeapons, RARNEY_PISTOL))
		{
			AlphaRarneyFirePistol();
		}
		...
		if (FBitSet(m_iWeapons, RARNEY_SNIPER))
		{
			AlphaRarneyFireSniper();
		}
		break;

		case ALPHARARNEY_AE_GRENADETHROW:
		{
...
/*	case ALPHARARNEY_AE_SHOTGUN:
		AlphaRarneyFireShotgun();
		break;
	case ALPHARARNEY_AE_SNIPER:
		AlphaRarneyFireSniper();
		break;
	case ALPHARARNEY_AE_SMG:
		AlphaRarneyFireSMG();
		break;
*/ //don't think these are necessary
	case ALPHARARNEY_AE_DRAW:
		// alphararney's bodygroup switches here so he can pull gun from holster
		if (FBitSet(m_iWeapons, RARNEY_PISTOL))
		{
			SetBodygroup( 1, ALPHARARNEY_BODY_GUNDRAWN);
		}
	...
	case ALPHARARNEY_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		SetBodygroup( 1, ALPHARARNEY_BODY_GUNHOLSTERED);
		m_fGunDrawn = false;
		break;
...
int CNPC_HL1AlphaRarney::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	case SCHED_ARM_WEAPON:
		if ( GetEnemy() != NULL )
		{
			...
			return SCHED_ALPHARARNEY_ENEMY_DRAW;
		}
		break;
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_ALPHARARNEY_FACE_TARGET;
			else
				return baseType;
		}
		break;
	...
	{
		return SCHED_ALPHARARNEY_FOLLOW;
		break;
	}
	...
	case SCHED_IDLE_STAND:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_ALPHARARNEY_IDLE_STAND;
			else
				return baseType;
		}
		break;
...
// of the proper type.
//=========================================================
int CNPC_HL1AlphaRarney::SelectSchedule( void )
{
...
void CNPC_HL1AlphaRarney::DeclineFollowing( void )
{
...
bool CNPC_HL1AlphaRarney::CanBecomeRagdoll( void )
{
	if ( UTIL_IsLowViolence() )
	{
		return false;
...
bool CNPC_HL1AlphaRarney::ShouldGib( const CTakeDamageInfo &info )
{
//	if ( UTIL_IsLowViolence() )
//	{
//		return false;
...
bool CNPC_HL1AlphaRarney::CorpseGib( const CTakeDamageInfo &info )
{
	CEffectData	data;
...
AI_BEGIN_CUSTOM_NPC( monster_alphararney, CNPC_HL1AlphaRarney )

	DECLARE_SQUADSLOT( ALPHARARNEY_SQUAD_SLOT_ATTACK1 )
	DECLARE_SQUADSLOT( ALPHARARNEY_SQUAD_SLOT_ATTACK2 )
	DECLARE_SQUADSLOT( ALPHARARNEY_SQUAD_SLOT_CHASE )
	DECLARE_SQUADSLOT( ALPHARARNEY_SQUAD_SLOT_INVESTIGATE_SOUND )

	//=========================================================
	// > SCHED_ALPHARARNEY_FOLLOW
	//=========================================================
	DEFINE_SCHEDULE
	(
		SCHED_ALPHARARNEY_FOLLOW,
		...
		"	Tasks"
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_ALPHARARNEY_STOP_FOLLOWING"
	...
	(
		...
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_ALPHARARNEY_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
...
//
//=========================================================
class CNPC_DeadAlphaRarney : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_DeadAlphaRarney, CAI_BaseNPC );
public:
...
char *CNPC_DeadAlphaRarney::m_szPoses[] = { "lying_on_back", "lying_on_side", "lying_on_stomach" };

bool CNPC_DeadAlphaRarney::KeyValue( const char *szKeyName, const char *szValue )
{
...
LINK_ENTITY_TO_CLASS( monster_alphararney_dead, CNPC_DeadAlphaRarney );

BEGIN_DATADESC( CNPC_DeadAlphaRarney )
END_DATADESC()

//=========================================================
// ********** DeadAlphaRarney SPAWN **********
//=========================================================
void CNPC_DeadAlphaRarney::Spawn( void )
{
	PrecacheModel("models/alphararneyhl1.mdl");
	SetModel( "models/alphararneyhl1.mdl");
	...
	if ( GetSequence() == -1 )
	{
		Msg ( "Dead alphararney with bad pose\n" );
	}
	// Corpses have less health
	m_iHealth			= 0.0;//gSkillData.alphararneyHealth;
