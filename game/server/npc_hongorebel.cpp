...
//#define HONGOREBEL_MODEL "models/citizen_17.mdl"

#define CONCEPT_PLAYER_USE "FollowWhenUsed"

ConVar	sk_hongorebel_health( "sk_hongorebel_health","50");

//=========================================================
//  Activities
//=========================================================

class CNPC_Hongorebel : public CNPC_PlayerCompanion
{
public:
	DECLARE_CLASS( CNPC_Hongorebel, CNPC_PlayerCompanion );
//	DECLARE_SERVERCLASS();
	DECLARE_DATADESC();
	...
	virtual void Precache()
	{
		...
		PrecacheModel( "models/Militia_Sniper.mdl" );
		PrecacheModel( "models/techgang.mdl" );
		PrecacheModel( "models/hongo_rebel.mdl" );


		PrecacheScriptSound( "NPC_Barney.FootstepLeft" );
		PrecacheScriptSound( "NPC_Barney.FootstepRight" );
		PrecacheScriptSound( "NPC_hongorebel.Die" );
...
LINK_ENTITY_TO_CLASS( npc_hongorebel, CNPC_Hongorebel );

//---------------------------------------------------------
// 
//---------------------------------------------------------
//IMPLEMENT_CUSTOM_AI( npc_hongorebel,CNPC_Hongorebel );


//IMPLEMENT_SERVERCLASS_ST(CNPC_Hongorebel, DT_NPC_Hongorebel)
//END_SEND_TABLE()
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_Hongorebel )
//						m_FuncTankBehavior
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Hongorebel::Spawn( void )
{
	Precache();

	SetModel ("models/Hongo_Rebel.mdl");

	m_iHealth = sk_hongorebel_health.GetFloat();;
	...
	SetUse( &CNPC_Hongorebel::FollowUse );
}
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CNPC_Hongorebel::Classify( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hongorebel::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Hongorebel::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Hongorebel::DeathSound( const CTakeDamageInfo &info )
{
	// Sentences don't play on dead NPCs
	SentenceStop();

	EmitSound( "npc_hongorebel.die" );

}

bool CNPC_Hongorebel::CreateBehaviors( void )
{
...
bool CNPC_Hongorebel::OnBehaviorChangeStatus( CAI_BehaviorBase *pBehavior, bool fCanFinishSchedule )
{
...
void CNPC_Hongorebel::OnChangeRunningBehavior( CAI_BehaviorBase *pOldBehavior,  CAI_BehaviorBase *pNewBehavior )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hongorebel::GatherConditions()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hongorebel::UseFunc( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
void CNPC_Hongorebel::FollowUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( m_pSquad && m_pSquad->NumMembers() > 1 )
	{
		...
		{
			CNPC_Hongorebel *pBarney = dynamic_cast<CNPC_Hongorebel *>(pSquadMember);
...
AI_BEGIN_CUSTOM_NPC( npc_hongorebel, CNPC_Hongorebel )
