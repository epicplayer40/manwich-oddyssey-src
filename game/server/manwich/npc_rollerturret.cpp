...
BEGIN_DATADESC( CNPC_RollerTurret )
	...
	DEFINE_FIELD( m_fIsOpen,	FIELD_BOOLEAN ),
...
void CNPC_RollerTurret::Precache( void )
{
	...
	engine->PrecacheModel( "sprites/muzzleflash2_unlit.vmt" );
	PrecacheScriptSound( "NPC_RollerTurret.Shoot" );
...
void CNPC_RollerTurret::ShootLaser( const Vector &vecTarget )
{
	...
	pLaser->Spawn( "sprites/muzzleflash2_unlit.vmt", GetLocalOrigin() + vecAim * 32, 
		vecAim * 1500, edict(), MOVETYPE_FLY, MOVECOLLIDE_DEFAULT, 1, DMG_SHOCK);
