...
#define SpeakerHL1_START_SILENT			1	// wait for trigger 'on' to start announcements

// ===================================================================================
//
// SpeakerHL1 class. Used for announcements per level, for door lock/unlock spoken voice. 
//

class CSpeakerHL1 : public CPointEntity
{
	DECLARE_CLASS( CSpeakerHL1, CPointEntity );
public:
	bool KeyValue( const char *szKeyName, const char *szValue );
	void Spawn( void );
	void Precache( void );
	void ToggleUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value );
	void SpeakerHL1Think( void );
...
LINK_ENTITY_TO_CLASS( SpeakerHL1, CSpeakerHL1 );

BEGIN_DATADESC( CSpeakerHL1 )
	DEFINE_FIELD( m_preset, FIELD_INTEGER ),
	DEFINE_KEYFIELD( m_iszMessage, FIELD_STRING, "message" ),
	DEFINE_THINKFUNC( SpeakerHL1Think ),
...
void CSpeakerHL1::Spawn( void )
{
	...
	SetThink(&CSpeakerHL1::SpeakerHL1Think);
	SetNextThink( TICK_NEVER_THINK );

	// allow on/off switching via 'use' function.
	SetUse ( &CSpeakerHL1::ToggleUse );
...
void CSpeakerHL1::Precache( void )
{
	if ( !FBitSet ( GetSpawnFlags(), SpeakerHL1_START_SILENT ) )
		// set first announcement time for random n second
		SetNextThink( gpGlobals->curtime + random->RandomFloat( 5.0, 15.0 ) );
}
void CSpeakerHL1::SpeakerHL1Think( void )
{
	...
	else
	{
		...
		if ( SENTENCEG_PlayRndSz( edict(), szSoundFile, flvolume, SNDLVL_120dB, flags, pitch) < 0 )
			 Msg(  "Level Design Error!\nSpeakerHL1 has bad sentence group name: %s\n",szSoundFile); 
...
//
void CSpeakerHL1::ToggleUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
bool CSpeakerHL1::KeyValue( const char *szKeyName, const char *szValue )
{
	// preset
