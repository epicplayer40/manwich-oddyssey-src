...
//4: Make a version with its own CLASS_ faction for each of the 5 variants for after the militia breaks off, monster_maluk_team1 with CLASS_TEAM1, monster_maluk_team2 with CLASS_TEAM2, etc. (Haven't started yet, it'll be tedious)
...
ConVar	sk_malukhl1_health( "sk_malukhl1_health","500");

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
// first flag is maluk dying for scripted sequences?
#define		MALUK_AE_DRAW		( 2 )
#define		MALUK_AE_SHOOT		( 3 )
#define		MALUK_AE_HOLSTER	( 4 )
#define		MALUK_AE_STOPLASER	( 5 )

#define		MALUK_BODY_GUNHOLSTERED	0
#define		MALUK_BODY_GUNDRAWN		1
#define		MALUK_BODY_GUNGONE			2


enum MalukSquadSlot_T
{	
	MALUK_SQUAD_SLOT_ATTACK1 = LAST_SHARED_SQUADSLOT,
	MALUK_SQUAD_SLOT_ATTACK2,
	MALUK_SQUAD_SLOT_INVESTIGATE_SOUND,
	MALUK_SQUAD_SLOT_CHASE,
};
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_HL1Maluk )
	DEFINE_FIELD( m_fGunDrawn, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_flPainTime, FIELD_TIME ),
	DEFINE_ARRAY( m_pBeam, FIELD_CLASSPTR, MALUK_MAX_BEAMS ),
...
LINK_ENTITY_TO_CLASS( monster_maluk, CNPC_HL1Maluk );
...
// Spawn
//=========================================================
void CNPC_HL1Maluk::Spawn()
{
	Precache( );

	SetModel( "models/squad_maluk.mdl");
	...
	m_iHealth			= sk_malukhl1_health.GetFloat();
	...
	SetUse( &CNPC_HL1Maluk::FollowerUse );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CNPC_HL1Maluk::Precache()
{
	m_iAmmoType = GetAmmoDef()->Index("Pistol");
	PrecacheModel( STRING( GetModelName() ) );

	PrecacheModel( "models/squad_maluk.mdl" );

	PrecacheScriptSound( "Maluk.FireWeapon" );
	PrecacheScriptSound( "Maluk.Pain" );
	PrecacheScriptSound( "Maluk.Die" );

	// every new maluk must call this, otherwise
	// when a level is loaded, nobody will talk (time is reset to 0)
	TalkInit();
	BaseClass::Precache();
}	

void CNPC_HL1Maluk::ModifyOrAppendCriteria( AI_CriteriaSet& criteriaSet )
{
...
// Init talk data
void CNPC_HL1Maluk::TalkInit()
{
	BaseClass::TalkInit();

	// get voice for head - just one maluk voice for now
...
//=========================================================
int CNPC_HL1Maluk::GetSoundInterests ( void) 
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_CARCASS	|
...
// relationship table.
//=========================================================
Class_T	CNPC_HL1Maluk::Classify ( void )
{
	return	CLASS_CONSCRIPT;
}

//=========================================================
// ALertSound - maluk says "Freeze!"
//=========================================================
void CNPC_HL1Maluk::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
	//	if ( IsOkToSpeak() )
...
//=========================================================
void CNPC_HL1Maluk::SetYawSpeed ( void )
{
	int ys;
...
// CheckRangeAttack1
//=========================================================
bool CNPC_HL1Maluk::CheckRangeAttack1 ( float flDot, float flDist )
{
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_HL1Maluk::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
void CNPC_HL1Maluk::ZapBeam( int side )
{
	Vector vecSrc, vecAim;
	trace_t tr;
	CBaseEntity *pEntity;

	if ( m_iBeams >= MALUK_MAX_BEAMS )
		 return;
	...
	float deflection = 0.02; //is this for accuracy??? 0.01 by default +++++update: Yep, it is.
	...
	m_pBeam[m_iBeams]->SetColor( 255, 0, 255 );
	m_pBeam[m_iBeams]->SetBrightness( 255 );
	m_pBeam[m_iBeams]->SetNoise( 0.0f );
...
//=========================================================
// MalukFirePistol - shoots one round from the pistol at
// the enemy maluk is facing.
//=========================================================
void CNPC_HL1Maluk::MalukFirePistol ( void )
{
...
int CNPC_HL1Maluk::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
// PainSound
//=========================================================
void CNPC_HL1Maluk::PainSound( const CTakeDamageInfo &info )
{
	...
	if ( GetParametersForSound( "Maluk.Pain", params, NULL ) )
	{
...
// DeathSound 
//=========================================================
void CNPC_HL1Maluk::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );

	CSoundParameters params;
	if ( GetParametersForSound( "Maluk.Die", params, NULL ) )
	{
		params.pitch = GetExpresser()->GetVoicePitch();
...
void CNPC_HL1Maluk::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_HL1Maluk::Event_Killed( const CTakeDamageInfo &info )
{
	if ( m_nBody < MALUK_BODY_GUNGONE )
	{
		// drop the gun!
		Vector vecGunPos;
		QAngle angGunAngles;
		...
		SetBodygroup( 1, MALUK_BODY_GUNGONE);
...
void CNPC_HL1Maluk::SUB_StartLVFadeOut( float delay, bool notSolid )
{
	SetThink( &CNPC_HL1Maluk::SUB_LVFadeOut );
...
void CNPC_HL1Maluk::SUB_LVFadeOut( void  )
{
...
void CNPC_HL1Maluk::StartTask( const Task_t *pTask )
{
		ClearBeams();
	BaseClass::StartTask( pTask );	
}

void CNPC_HL1Maluk::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
void CNPC_HL1Maluk::ClearBeams( )
{
	for (int i = 0; i < MALUK_MAX_BEAMS; i++)
	{
		if (m_pBeam[i])
		{
...
void CNPC_HL1Maluk::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
	case MALUK_AE_SHOOT:
	...
	case MALUK_AE_DRAW:
		// maluk's bodygroup switches here so he can pull gun from holster
		SetBodygroup( 1, MALUK_BODY_GUNDRAWN);
		m_fGunDrawn = true;
		break;

	case MALUK_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		SetBodygroup( 1, MALUK_BODY_GUNHOLSTERED);
		m_fGunDrawn = false;
		break;
		case MALUK_AE_STOPLASER:
		{
			ClearBeams();
		}
		break;
...
int CNPC_HL1Maluk::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_MALUK_FACE_TARGET;
			else
				return baseType;
		}
		break;
...
// of the proper type.
//=========================================================
int CNPC_HL1Maluk::SelectSchedule( void )
{
...
void CNPC_HL1Maluk::DeclineFollowing( void )
{
...
bool CNPC_HL1Maluk::CanBecomeRagdoll( void )
{
	if ( UTIL_IsLowViolence() )
	{
		return false;
...
bool CNPC_HL1Maluk::ShouldGib( const CTakeDamageInfo &info )
{
//	if ( UTIL_IsLowViolence() )
//	{
//		return false;
...
bool CNPC_HL1Maluk::CorpseGib( const CTakeDamageInfo &info )
{
	CEffectData	data;
...
AI_BEGIN_CUSTOM_NPC( monster_maluk, CNPC_HL1Maluk )

	DECLARE_SQUADSLOT( MALUK_SQUAD_SLOT_ATTACK1 )
	DECLARE_SQUADSLOT( MALUK_SQUAD_SLOT_ATTACK2 )
	DECLARE_SQUADSLOT( MALUK_SQUAD_SLOT_CHASE )
	DECLARE_SQUADSLOT( MALUK_SQUAD_SLOT_INVESTIGATE_SOUND )
	...
	(
		...
		"	Tasks"
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_MALUK_STOP_FOLLOWING"
	...
	(
		...
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_MALUK_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
