...
void CGrenade_Molotov::Spawn( void )
{
	...
	SetModel( "models/props_junk/GlassBottle01a.mdl");
	...
	if( m_pFireTrail )
	{
		...
		m_pFireTrail->m_EndColor.Init( 255.0, 200.0, 150.0 );
...
// Start some fires
	...
	for( i = 0 ; i < 16 ; i++ )
	{
		...
		//Get our scale based on distance - scale-growth is originally 0.1f
		float scale	 = 125.0f + ( 0.75f * ( 1.0f - ( offset / 128.0f ) ) );
		float growth = 125.0f + ( 0.75f * ( offset / 128.0f ) );

		if( firetrace.fraction != 1.0 )
		{//originally 30.0f
			FireSystem_StartFire( firetrace.endpos, scale, growth, 30.0f, (SF_FIRE_START_ON | SF_FIRE_SMOKELESS), (CBaseEntity*) this, FIRE_NATURAL );
		}
	}
...
void CGrenade_Molotov::Precache( void )
{
	...
	PrecacheModel("models/props_junk/GlassBottle01a.mdl");
