...
BEGIN_DATADESC( CNPC_RollerDozer )
	...
	DEFINE_FIELD( m_flTimeDebrisSearch,	FIELD_TIME ),
	DEFINE_FIELD( m_hDebris,				FIELD_EHANDLE ),
	DEFINE_FIELD( m_vecCleanupPoint,		FIELD_VECTOR ),
...
{
	return(pHint->HintType() == HINT_NOT_USED_ROLLER_CLEANUP_POINT);
}
...
CBaseEntity *CNPC_RollerDozer::FindDebris( void )
{
	if( !GetHintNode() )
	{
		...
		CAI_Hint* pHintNode = CAI_HintManager::FindHint( this, HINT_NOT_USED_ROLLER_CLEANUP_POINT, 0, 1024 ); 
