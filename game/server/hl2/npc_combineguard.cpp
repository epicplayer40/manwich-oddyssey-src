...
void CNPC_CombineGuard::Precache( void )
{
	...
	PrecacheModel( "models/combine_guard_ally.mdl" );
...
void CNPC_CombineGuard::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
		...
		DispatchParticleEffect( "blood_impact_synth_01_arc_parent", PATTACH_POINT_FOLLOW, this, 1 );
...
//	Vector impactPoint = ( ( GetEnemy()->BodyTarget( GetAbsOrigin() ) ) + vecShootDir ); //perfect accuracy. Gotta figure things out still
	QAngle angDir;
