...
class C_ImmolatorBeam : public C_BaseCombatCharacter
{
	DECLARE_CLASS( C_ImmolatorBeam, C_BaseCombatCharacter );
	DECLARE_CLIENTCLASS();
public:
	
	C_ImmolatorBeam( void );
...
private:
	...
	C_ImmolatorBeam( const C_ImmolatorBeam & ); // not defined, not accessible

	Vector	m_vecLastOrigin;
	bool	m_bUpdated;
};

IMPLEMENT_CLIENTCLASS_DT( C_ImmolatorBeam, DT_ImmolatorBeam, CImmolatorBeam )
END_RECV_TABLE()

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
C_ImmolatorBeam::C_ImmolatorBeam( void )
{
}
...
// Input  : updateType - 
//-----------------------------------------------------------------------------
void C_ImmolatorBeam::OnDataChanged( DataUpdateType_t updateType )
{
	BaseClass::OnDataChanged( updateType );
...
// Output : int
//-----------------------------------------------------------------------------
int C_ImmolatorBeam::DrawModel( int flags )
{
	// See if we're drawing the motion blur
	if ( flags & STUDIO_TRANSPARENCY )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_ImmolatorBeam::ClientThink( void )
{
	m_bUpdated = false;
}