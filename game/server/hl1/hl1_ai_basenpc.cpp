...
bool CHL1BaseNPC::HasAlienGibs( void )
{
	...
	if ( myClass == CLASS_ANTLION || 
		myClass == CLASS_MILITARY ||
		myClass == CLASS_ZOMBIE ||
		myClass == CLASS_HOUNDEYE ||
		myClass == CLASS_SHOCKTROOPER )
...
bool CHL1BaseNPC::HasCrematorGibs( void )
{
	Class_T myClass = Classify();

	if ( myClass == CLASS_COMBINE )
