...
CParticleMgr* g_ParticleMgr = ParticleMgr();

#define CGUARD_CHARGE_TIME 2.0f//Lychy: use this to change the charge time 
...
void CSphereTrails::AddStreaks(float floatCount)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		pStreak->m_flColor[0] = colorRamp;
		pStreak->m_flColor[1] = colorRamp;
		pStreak->m_flColor[2] = 1.0f;
		pStreak->m_flColor[3] = 1.0f;
	}
}
...
bool CSphereTrails::SimulateAndRender(Particle* pInParticle, ParticleDraw* pDraw, float& sortKey)
{
	...
	color[0] = pParticle->m_color.r * ramp;
	color[1] = pParticle->m_color.g * ramp;
	color[2] = pParticle->m_color.b * ramp;
	color[3] = pParticle->m_color.a * ramp;
...
void CSphereTrails::SimulateParticles(CParticleSimulateIterator* pIterator)
{
	...
	while (pParticle)
	{
		...
		pParticle->m_timeDelta = pIterator->GetTimeDelta();
		//Should this particle die?
		pParticle->m_flLifetime += pParticle->m_timeDelta;
		...
		timeDelta = pParticle->m_timeDelta;
...
void CSphereTrails::RenderParticles(CParticleRenderIterator* pIterator)
{
	...
	//Simulate the movement with collision
	//Lychy: sometimes the delta is nan, so heres a lil fix
	if (pParticle->m_timeDelta != pParticle->m_timeDelta)
		pParticle->m_timeDelta = gpGlobals->frametime;
	pParticle->m_Pos += pParticle->m_vecVelocity * pParticle->m_timeDelta;
	pParticle = (CSTParticle*)pIterator->GetNext(sortKey);
}
...
#define BALL_GROW_TIME	(CGUARD_CHARGE_TIME*0.9f)
#define BALL_RELEASE_TIME	(CGUARD_CHARGE_TIME*0.1f)
...
public:
	...
	virtual void RenderParticles(CParticleRenderIterator* pIterator) override
	{
		...
		while (pParticle)
		{
			...
			pParticle = (CCombineGuardBallParticle*)pIterator->GetNext(sortKey);
		}
		return;
	...
	virtual void Update(float dt)
	{
		...
		if (gpGlobals->curtime > m_killTime)
			return;	
		m_clock += dt;
...
static void DrawSphere(int stacks, int slices, float radius, Vector& origin)
{
	...
	CMatRenderContextPtr pMaterials(materials);
	IMesh* pMesh = pMaterials->GetDynamicMesh();
...
public:
	...
	virtual void Draw(bool bCacheFullSceneState)
	{
		...
		pContext->Bind(m_pWarpMaterial, NULL);
...
void C_CombineGuard::ReceiveMessage(int classID, bf_read& msg)
{
	...
	case CGUARD_MSG_SHOT_START:
	{
		...
		if (CCombineGuardMuzzleOverlay* pOverlay = new CCombineGuardMuzzleOverlay)
		{
			...
			pOverlay->m_flTotalLifetime = CGUARD_CHARGE_TIME;  // seconds
			//bool found;
			pOverlay->m_pWarpMaterial = materials->FindMaterial("sprites/predator_refract", TEXTURE_GROUP_PARTICLE, true);
			pOverlay->Activate();
		}
	}
	break;
