...
//
// Purpose: The Race X Shocktroopers, stranded on Earth after the Black Mesa incident. 
//
//
// $Workfile:     $
// $Date:         $
...
#include	"AI_Motor.h"
#include	"hl1_npc_shocktrooper.h"
...
ConVar	sk_strooper_health("sk_strooper_health", "200");
ConVar  sk_strooper_kick("sk_strooper_kick", "10");
ConVar  sk_strooper_pellets("sk_strooper_pellets", "7");
ConVar  sk_strooper_gspeed("sk_strooper_gspeed", "600");
ConVar	sk_strooper_dmgball("sk_strooper_dmgball", "7");

extern ConVar sk_plr_dmg_grenade;

extern ConVar sk_antlion_worker_spit_grenade_dmg;
ConVar sk_strooper_spore_grenade_radius("sk_strooper_spore_grenade_radius", "180");

#define SF_STROOPER_LEADER	( 1 << 5  )

extern int g_fGruntQuestion;				// true if an idle grunt asked a question. Cleared when someone answers.
extern int g_iSquadIndex;

#define STROOPER_GUN_SPREAD 0.08716f

//=========================================================
// monster-specific DEFINE's
//=========================================================
#define	STROOPER_CLIP_SIZE					36 // how many bullets in a clip? - NOTE: 3 round burst sound, so keep as 3 * x!
#define STROOPER_VOL						0.35		// volume of grunt sounds
#define STROOPER_SNDLVL					SNDLVL_TALKING	// soundlevel of grunt sentences
#define STROOPER_LIMP_HEALTH				sk_strooper_health.GetFloat() / 2 //Limp when at half health
...
#define	STROOPER_SENTENCE_VOLUME			(float)1.0 // volume of grunt sentences

#define STROOPER_9MMAR				( 1 << 0)
#define STROOPER_HANDGRENADE			( 1 << 1)
#define STROOPER_GRENADELAUNCHER		( 1 << 2)
#define STROOPER_SHOTGUN				( 1 << 3)

#define HEAD_GROUP					1
#define HEAD_STROOPER					0
...
const char *CNPC_STrooper::pGruntSentences[] =
{
...
LINK_ENTITY_TO_CLASS(monster_shocktrooper, CNPC_STrooper);
LINK_ENTITY_TO_CLASS(monster_shocktrooper_Ally, CNPC_STrooper);
...
enum
{
	SCHED_STROOPER_FAIL = LAST_SHARED_SCHEDULE,
	SCHED_STROOPER_COMBAT_FAIL,
	SCHED_STROOPER_VICTORY_DANCE,
	SCHED_STROOPER_ESTABLISH_LINE_OF_FIRE,
	SCHED_STROOPER_ESTABLISH_LINE_OF_FIRE_RETRY,
	SCHED_STROOPER_FOUND_ENEMY,
	SCHED_STROOPER_COMBAT_FACE,
	SCHED_STROOPER_SIGNAL_SUPPRESS,
	SCHED_STROOPER_SUPPRESS,
	SCHED_STROOPER_WAIT_IN_COVER,
	SCHED_STROOPER_TAKE_COVER,
	SCHED_STROOPER_GRENADE_COVER,
	SCHED_STROOPER_TOSS_GRENADE_COVER,
	SCHED_STROOPER_HIDE_RELOAD,
	SCHED_STROOPER_SWEEP,
	SCHED_STROOPER_RANGE_ATTACK1A,
	SCHED_STROOPER_RANGE_ATTACK1B,
	SCHED_STROOPER_RANGE_ATTACK2,
	SCHED_STROOPER_REPEL,
	SCHED_STROOPER_REPEL_ATTACK,
	SCHED_STROOPER_REPEL_LAND,
	SCHED_STROOPER_TAKE_COVER_FAILED,
	SCHED_STROOPER_RELOAD,
	SCHED_STROOPER_TAKE_COVER_FROM_ENEMY,
	SCHED_STROOPER_BARNACLE_HIT,
	SCHED_STROOPER_BARNACLE_PULL,
	SCHED_STROOPER_BARNACLE_CHOMP,
	SCHED_STROOPER_BARNACLE_CHEW,
};
...
enum
{
	TASK_STROOPER_FACE_TOSS_DIR = LAST_SHARED_TASK + 1,
...
enum
{
	COND_STROOPER_NOFIRE = LAST_SHARED_CONDITION + 1,
};
...
int ACT_STROOPER_LAUNCH_GRENADE;
int ACT_STROOPER_TOSS_GRENADE;
int ACT_STROOPER_MP5_STANDING;
int ACT_STROOPER_MP5_CROUCHING;
int ACT_STROOPER_SHOTGUN_STANDING;
int ACT_STROOPER_SHOTGUN_CROUCHING;

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CNPC_STrooper)
...
// Spawn
//=========================================================
void CNPC_STrooper::Spawn()
{
	Precache();

	SetModel("models/strooper.mdl");
	...
	ClearEffects();
	m_iHealth = sk_strooper_health.GetFloat();
	...
	//	m_iWeapons = STROOPER_SHOTGUN | STROOPER_HANDGRENADE;
	//	m_iWeapons = STROOPER_HANDGRENADE;
		// pev->weapons = STROOPER_SHOTGUN;
		// pev->weapons = STROOPER_9MMAR | STROOPER_GRENADELAUNCHER;
		...
		if (r == 1)
		{*/
			m_iWeapons = STROOPER_9MMAR | STROOPER_HANDGRENADE;
		/*}
		else if (r == 2)
		{
			m_iWeapons = STROOPER_SHOTGUN | STROOPER_HANDGRENADE;
	...
	if (FBitSet(m_iWeapons, STROOPER_SHOTGUN))
	{
	...
	{
		m_iClipSize = STROOPER_CLIP_SIZE;
	}
...
//	else
	...
	else if (FBitSet(m_iWeapons, STROOPER_GRENADELAUNCHER))
	{
	...
	SetUse( &CNPC_STrooper::FollowerUse );
}

//=========================================================
// Strooper Projectile
//=========================================================
class CStrooperProjectile : public CBaseEntity
{
	DECLARE_CLASS( CStrooperProjectile, CBaseEntity );
public:
	void Spawn( void );
	void Precache( void );
	...
	int m_nStrooperProjectileSprite;
...
LINK_ENTITY_TO_CLASS( strooper_energy_ball, CStrooperProjectile );

BEGIN_DATADESC( CStrooperProjectile )
	DEFINE_FIELD( m_nStrooperProjectileSprite, FIELD_INTEGER ),
	DEFINE_FIELD( m_hSprite, FIELD_EHANDLE ),
END_DATADESC()


void CStrooperProjectile::Precache( void )
{
	m_nStrooperProjectileSprite = PrecacheModel("sprites/blueflare.vmt");// client side spittle.

	PrecacheScriptSound( "Strooper.ProjImpact" );
	PrecacheScriptSound( "NPC_BigMomma.SpitHit1" );
	PrecacheScriptSound( "NPC_BigMomma.SpitHit2" );
}

void CStrooperProjectile::Spawn( void )
{
	Precache();

	SetMoveType ( MOVETYPE_FLY );
	SetClassname( "strooper_energy_ball" );
	...
	SetSprite( CSprite::SpriteCreate( "sprites/blueflare.vmt", GetAbsOrigin(), true ) );
...
void CStrooperProjectile::Shoot( CBaseEntity *pOwner, Vector vecStart, Vector vecVelocity )
{
	CStrooperProjectile *pSpit = CREATE_ENTITY( CStrooperProjectile, "strooper_energy_ball" );
	pSpit->Spawn();
	...
	if ( pSprite )
	{
		...
		pSprite->SetColor( 0, 0, 255 );

		pSprite->SetScale( 0.23 );
...
void CStrooperProjectile::Touch( CBaseEntity *pOther )
{
	trace_t tr;
	int		iPitch;
	...
	if ( !pOther->m_takedamage )
	{
		...
		// make some flecks --LET'S NOT
		CPVSFilter filter( tr.endpos );

	}
	else
	{
		CTakeDamageInfo info( this, this, sk_strooper_dmgball.GetFloat(), DMG_SHOCK );
		//CalculateBulletDamageForce( &info, GetAmmoDef()->Index("9mmRound"), GetAbsVelocity(), GetAbsOrigin() );
		pOther->TakeDamage( info );
	}
	UTIL_Remove( m_hSprite );
	UTIL_Remove( this );
	//DispatchParticleEffect( "hunter_projectile_explosion_1", GetAbsOrigin(), vecAngles );
}




void CNPC_STrooper::FollowerUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
//		}
	...
	CNPC_STrooper::FollowerUse( pActivator, pCaller, useType, value );
}


int CNPC_STrooper::IRelationPriority(CBaseEntity *pTarget)
{
...
//=========================================================
void CNPC_STrooper::Precache()
{
	m_iAmmoType = GetAmmoDef()->Index("buckshot");

	PrecacheModel("models/strooper.mdl");
	...
	PrecacheScriptSound("STrooper.Reload");
	PrecacheScriptSound("STrooper.GrenadeLaunch");
	PrecacheScriptSound("STrooper.Fire");
	PrecacheScriptSound("STrooper.ProjImpact");
	PrecacheScriptSound("STrooper.Shotgun");
	PrecacheScriptSound("STrooper.Pain");
	PrecacheScriptSound("STrooper.Die");
	PrecacheScriptSound( "NPC_Antlion.PoisonShoot" );
	PrecacheScriptSound( "NPC_Antlion.PoisonBall" );

	BaseClass::Precache();

	UTIL_PrecacheOther("grenade_spit_hl2");
	UTIL_PrecacheOther("grenade_spit");
	UTIL_PrecacheOther("strooper_energy_ball");
...
//=========================================================
bool CNPC_STrooper::FOkToSpeak(void)
{
...
// started moving.
//=========================================================
void CNPC_STrooper::SpeakSentence(void)
{
	...
	if (FOkToSpeak())
	{
		SENTENCEG_PlayRndSz(edict(), pGruntSentences[m_iSentence], STROOPER_SENTENCE_VOLUME, STROOPER_SNDLVL, 0, m_voicePitch);
		JustSpoke();
	}
}
...
//=========================================================
//=========================================================
void CNPC_STrooper::JustSpoke(void)
{
...
//=========================================================
void CNPC_STrooper::PrescheduleThink(void)
{
	...
	if (m_pSquad && GetEnemy() != NULL)
	{
		...
		CNPC_STrooper *pSquadLeader = (CNPC_STrooper*)m_pSquad->GetLeader()->MyNPCPointer();
...
Class_T	CNPC_STrooper::Classify(void)
{
	if (FClassnameIs(this, "monster_shocktrooper"))
	{
		return	CLASS_SHOCKTROOPER;
	}

	if (FClassnameIs(this, "monster_shocktrooper_ally"))
	{
...
//
//=========================================================
int CNPC_STrooper::SquadRecruit(int searchRadius, int maxMembers)
{
	int squadCount;
	...
	if (m_SquadName != NULL_STRING)
	{
		...
		pEntity = gEntList.FindEntityByClassname(pEntity, "monster_shocktrooper");

		while (pEntity)
		{
			CNPC_STrooper *pRecruit = (CNPC_STrooper*)pEntity->MyNPCPointer();
	...
	else
	{
		...
		{
			if (!FClassnameIs(pEntity, "monster_shocktrooper"))
				continue;
...
void CNPC_STrooper::StartNPC(void)
{
	if (!m_pSquad)
	{
		if (m_SquadName != NULL_STRING)
		{
			...
			if (GetSpawnFlags() & SF_STROOPER_LEADER)
			{
				InitSquad();
...
// CheckMeleeAttack1
//=========================================================
int CNPC_STrooper::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (flDist > 64)
...
//=========================================================
int CNPC_STrooper::RangeAttack1Conditions(float flDot, float flDist)
{
...
int CNPC_STrooper::RangeAttack2Conditions(float flDot, float flDist)
{
	m_iLastGrenadeCondition = GetGrenadeConditions(flDot, flDist);
	return m_iLastGrenadeCondition;
}

int CNPC_STrooper::GetGrenadeConditions(float flDot, float flDist)
{
	if (!FBitSet(m_iWeapons, (STROOPER_HANDGRENADE | STROOPER_GRENADELAUNCHER)))
		return COND_NONE;
	...
	if (FBitSet(m_iWeapons, STROOPER_HANDGRENADE))
	{
		// find feet
		if (random->RandomInt(0, 1))
		{
	...
	else
	{
		...
		if (HasCondition(COND_SEE_ENEMY))
		{
			vecTarget = vecTarget + ((vecTarget - GetAbsOrigin()).Length() / sk_strooper_gspeed.GetFloat()) * GetEnemy()->GetAbsVelocity();
		}
	}
	...
	else
	{
		...
		Vector vecToss = VecCheckThrow(this, vGunPos, vecTarget, sk_strooper_gspeed.GetFloat(), 0.5);
...
//=========================================================
bool CNPC_STrooper::FCanCheckAttacks(void)
{
...
int CNPC_STrooper::GetSoundInterests(void)
{
	return	SOUND_WORLD |
		SOUND_COMBAT |
		SOUND_PLAYER |
...
//=========================================================
void CNPC_STrooper::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr)
{
	...
	if (ptr->hitgroup == 11)
	{
		...
		if (GetBodygroup(1) == HEAD_STROOPER && (info.GetDamageType() & (DMG_BULLET | DMG_SLASH | DMG_BLAST | DMG_CLUB)))
		{
			// absorb damage
...
//=========================================================
int CNPC_STrooper::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
...
//	}
	...
	CNPC_STrooper::StartFollowing( pLeader );
}

void CNPC_STrooper::StopFollowing( void )
{

	CNPC_STrooper::StopFollowing();
}
...
//=========================================================
float CNPC_STrooper::MaxYawSpeed(void)
{
	float flYS;
...
void CNPC_STrooper::IdleSound(void)
{
	if (FOkToSpeak() && (g_fGruntQuestion || random->RandomInt(0, 1)))
	{
		if (!g_fGruntQuestion)
		{
			...
			{
			case 0: // check in
				SENTENCEG_PlayRndSz(edict(), "ST_CHECK", STROOPER_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				g_fGruntQuestion = 1;
				break;
			case 1: // question
				SENTENCEG_PlayRndSz(edict(), "ST_QUEST", STROOPER_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				g_fGruntQuestion = 2;
				break;
			case 2: // statement
				SENTENCEG_PlayRndSz(edict(), "ST_IDLE", STROOPER_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			}
		}
		else
		{
			...
			{
			case 1: // check in
				SENTENCEG_PlayRndSz(edict(), "ST_CLEAR", STROOPER_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			case 2: // question 
				SENTENCEG_PlayRndSz(edict(), "ST_ANSWER", STROOPER_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			}
			g_fGruntQuestion = 0;
		}
...
bool CNPC_STrooper::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_STrooper::CheckAmmo(void)
{
	if (m_cAmmoLoaded <= 0)
...
//=========================================================
//=========================================================
CBaseEntity *CNPC_STrooper::Kick(void)
{
	trace_t tr;
...
Vector CNPC_STrooper::Weapon_ShootPosition(void)
{
	if (m_fStanding)
...
void CNPC_STrooper::Event_Killed(const CTakeDamageInfo &info)
{
	Vector	vecGunPos;
	QAngle	vecGunAngles;
	...
	{
		DropItem("monster_shockroach", GetAbsOrigin(), GetAbsAngles());
	}

	if (FBitSet(m_iWeapons, STROOPER_GRENADELAUNCHER))
	{
...
//=========================================================
void CNPC_STrooper::HandleAnimEvent(animevent_t *pEvent)
{
	Vector	vecShootDir;
	...
	case STROOPER_AE_RELOAD:
	{
		...
		EmitSound(filter, entindex(), "STrooper.Reload");
		...
		if (pGrenade)
		{
			pGrenade->ShootTimed(this, m_vecTossVelocity, 3.5);
				...
				CGrenadeSpit *pGrenade = (CGrenadeSpit*) CreateEntityByName( "grenade_spit" );
					pGrenade->SetAbsOrigin( GetAbsOrigin() + Vector(0, 0, 60) );
					...
					pGrenade->SetDamage(sk_antlion_worker_spit_grenade_dmg.GetFloat());
					pGrenade->SetGravity( UTIL_ScaleForGravity( 350 ) );	// use a lower gravity for grenades to make them easier to see
	...
	case STROOPER_AE_GREN_LAUNCH:
	{
		...
		EmitSound(filter2, entindex(), "STrooper.GrenadeLaunch");
	...
	{
		CHandGrenade *pGrenade = (CHandGrenade*)Create("grenade_spit", Weapon_ShootPosition(), vec3_angle); //Oroginally grenade_spit... Broke as fuck
		if (pGrenade)
		{
		/*	pGrenade->ShootTimed(this, m_vecTossVelocity, 3.5);
			pGrenade->SetDamageRadius(612);
			pGrenade->SetDamage(sk_antlion_worker_spit_grenade_dmg.GetFloat());
			pGrenade->SetHealth(5);*/
	...
	{
		if (FBitSet(m_iWeapons, STROOPER_9MMAR))
		{
				...
				vecSpitDir.x += random->RandomFloat( -0.01, 0.01 );
				vecSpitDir.y += random->RandomFloat( -0.01, 0.01 );
				vecSpitDir.z += random->RandomFloat( -0.01, 0 );
			
				CStrooperProjectile::Shoot( this, vecSpitOffset, vecSpitDir * 900 );

		//	Shoot();

			CBroadcastRecipientFilter filter;
			te->DynamicLight( filter, 0.0, &GetAbsOrigin(), 0, 146, 255, 2, 128
				, 0.2 / m_flPlaybackRate, 0 );

			CPASAttenuationFilter filter3(this);
			// the first round of the three round burst plays the sound and puts a sound in the world sound list.
			EmitSound(filter3, entindex(), "STrooper.Fire");
		}
		else
		{
			Shotgun();
			...
			EmitSound(filter4, entindex(), "STrooper.Shotgun");
		}
	...
	case STROOPER_AE_KICK:
	{
		...
		if (pHurt)
		{
			...
			if (pHurt->entindex() > 0)
			{
				...
				CTakeDamageInfo info(this, this, sk_strooper_kick.GetFloat(), DMG_CLUB);
	...
	case STROOPER_AE_CAUGHT_ENEMY:
	{
		if (FOkToSpeak())
		{
			SENTENCEG_PlayRndSz(edict(), "ST_ALERT", STROOPER_SENTENCE_VOLUME, STROOPER_SNDLVL, 0, m_voicePitch);
			JustSpoke();
		}
...
void CNPC_STrooper::SetAim(const Vector &aimDir)
{
	QAngle angDir;
...
// Shoot
//=========================================================
void CNPC_STrooper::Shotgun(void)
{
	if (GetEnemy() == NULL)
		return;
	...
	FireBullets(sk_strooper_pellets.GetFloat(), vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 2048, m_iAmmoType, 0); // shoot +-7.5 degrees
...
// start task
//=========================================================
void CNPC_STrooper::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_STROOPER_CHECK_FIRE:
		if (!NoFriendlyFire())
		{
	...
	case TASK_STROOPER_SPEAK_SENTENCE:
		SpeakSentence();
		TaskComplete();
		break;
	...
	case TASK_STROOPER_FACE_TOSS_DIR:
		break;
...
// RunTask
//=========================================================
void CNPC_STrooper::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_STROOPER_FACE_TOSS_DIR:
...
// PainSound
//=========================================================
void CNPC_STrooper::PainSound(const CTakeDamageInfo &info)
{
	if (gpGlobals->curtime > m_flNextPainTime)
	{
		CPASAttenuationFilter filter(this);
		EmitSound(filter, entindex(), "STrooper.Pain");
...
// DeathSound 
//=========================================================
void CNPC_STrooper::DeathSound(const CTakeDamageInfo &info)
{
	CPASAttenuationFilter filter(this, ATTN_IDLE);
	EmitSound(filter, entindex(), "STrooper.Die");
}

//=========================================================
// SetActivity 
//=========================================================
Activity CNPC_STrooper::NPC_TranslateActivity(Activity eNewActivity)
{
	switch (eNewActivity)
	{
	case ACT_RANGE_ATTACK1:
		...
		if (FBitSet(m_iWeapons, STROOPER_9MMAR))
		{
			if (m_fStanding)
			{
				...
				return (Activity)ACT_STROOPER_MP5_STANDING;
			}
			else
			{
				// get crouching shoot
				return (Activity)ACT_STROOPER_MP5_CROUCHING;
			}
		}
		else
		{
			...
			{
				// get aimable sequence
				return (Activity)ACT_STROOPER_SHOTGUN_STANDING;
			}
			else
			{
				// get crouching shoot
				return (Activity)ACT_STROOPER_SHOTGUN_CROUCHING;
			}
		}
		break;
	case ACT_RANGE_ATTACK2:
		...
		if (m_iWeapons & STROOPER_HANDGRENADE)
		{
			// get toss anim
			return (Activity)ACT_STROOPER_TOSS_GRENADE;
		}
		else
		{
			// get launch anim
			return (Activity)ACT_STROOPER_LAUNCH_GRENADE;
		}
		break;
	case ACT_RUN:
		if (m_iHealth <= STROOPER_LIMP_HEALTH)
		{
			// limp!
			return ACT_RUN_HURT;
		}
...
void CNPC_STrooper::ClearAttackConditions(void)
{
...
int CNPC_STrooper::SelectSchedule(void)
{
	// clear old sentence
	m_iSentence = STROOPER_SENT_NONE;
	...
	if (GetMoveType() == MOVETYPE_FLYGRAVITY && m_NPCState != NPC_STATE_PRONE)
	{
		if (GetFlags() & FL_ONGROUND)
		{
			...
			SetMoveType(MOVETYPE_STEP);
			SetGravity(1.0);
			return SCHED_STROOPER_REPEL_LAND;
		}
		else
		{
			// repel down a rope, 
			if (m_NPCState == NPC_STATE_COMBAT)
				return SCHED_STROOPER_REPEL_ATTACK;
			else
				return SCHED_STROOPER_REPEL;
		}
	}
	...
	if (HasCondition(COND_HEAR_DANGER))
	{
		...
		if (FOkToSpeak())
		{
			SENTENCEG_PlayRndSz(edict(), "ST_GREN", STROOPER_SENTENCE_VOLUME, STROOPER_SNDLVL, 0, m_voicePitch);
			JustSpoke();
		}
	...
	case NPC_STATE_PRONE:
	{
		...
		{
			return SCHED_STROOPER_BARNACLE_CHOMP;
		}
		else
		{
			return SCHED_STROOPER_BARNACLE_HIT;
		}
	}
	...
	case NPC_STATE_COMBAT:
	{
				...
				else
				{
					...
					if (FOkToSpeak())// && RANDOM_LONG(0,1))
					{
						if ((GetEnemy() != NULL) )
							...
							// monster
							SENTENCEG_PlayRndSz(edict(), "ST_MONST", STROOPER_SENTENCE_VOLUME, STROOPER_SNDLVL, 0, m_voicePitch);
					...
					{
						return SCHED_STROOPER_SUPPRESS;
					}
					else
					{
						return SCHED_STROOPER_ESTABLISH_LINE_OF_FIRE;
					}
				}
			}
		}
		...
		else if (HasCondition(COND_NO_PRIMARY_AMMO))
		{
			...
			return SCHED_STROOPER_HIDE_RELOAD;
		}
		...
		else if (HasCondition(COND_LIGHT_DAMAGE))
		{
			...
			if (iPercent <= 90 && GetEnemy() != NULL)
			{
				...
				if (FOkToSpeak()) // && RANDOM_LONG(0,1))
				{
					SENTENCEG_PlayRndSz(edict(), "ST_COVER", STROOPER_SENTENCE_VOLUME, STROOPER_SNDLVL, 0, m_voicePitch);
		...
		else if (FBitSet(m_iWeapons, STROOPER_GRENADELAUNCHER) && HasCondition(COND_CAN_RANGE_ATTACK2) && OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2))
		{
		...
		else if (HasCondition(COND_CAN_RANGE_ATTACK1))
		{
			if (m_pSquad)
			{
				if (m_pSquad->GetLeader() != NULL)
				{
					...
					{
						return SCHED_STROOPER_FOUND_ENEMY;
					}
				}
			}
		...
		else if (HasCondition(COND_ENEMY_OCCLUDED))
		{
			if (HasCondition(COND_CAN_RANGE_ATTACK2) && OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2))
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(edict(), "ST_THROW", STROOPER_SENTENCE_VOLUME, STROOPER_SNDLVL, 0, m_voicePitch);
					JustSpoke();
				}
				return SCHED_RANGE_ATTACK2;
			}
			else if (OccupyStrategySlotRange(SQUAD_SLOT_ENGAGE1, SQUAD_SLOT_ENGAGE2))
			{
				...
				if (FOkToSpeak())// && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "ST_CHARGE", STROOPER_SENTENCE_VOLUME, STROOPER_SNDLVL, 0, m_voicePitch);
			...
			else
			{
				...
				{
					SENTENCEG_PlayRndSz(edict(), "ST_TAUNT", STROOPER_SENTENCE_VOLUME, STROOPER_SNDLVL, 0, m_voicePitch);
					JustSpoke();
				}
...
int CNPC_STrooper::TranslateSchedule(int scheduleType)
{
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
	{
		if (m_pSquad)
		{
			if (g_iSkillLevel == SKILL_HARD && HasCondition(COND_CAN_RANGE_ATTACK2) && OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2))
			{
				...
				return SCHED_STROOPER_TOSS_GRENADE_COVER;
			}
			else
			{
				return SCHED_STROOPER_TAKE_COVER;
			}
		}
		else
		{
			...
			{
				return SCHED_STROOPER_GRENADE_COVER;
			}
		}
	}
	case SCHED_STROOPER_TAKE_COVER_FAILED:
	{
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_STROOPER_FACE_TARGET;
			else
				return baseType;
		}
		break;
	...
	case SCHED_RANGE_ATTACK1:
	{
		...
		if (m_fStanding)
			return SCHED_STROOPER_RANGE_ATTACK1B;
		else
			return SCHED_STROOPER_RANGE_ATTACK1A;
	}

	case SCHED_RANGE_ATTACK2:
	{
		return SCHED_STROOPER_RANGE_ATTACK2;
	}
	case SCHED_VICTORY_DANCE:
	{
		if (m_pSquad)
		{
			if (!m_pSquad->IsLeader(this))
			{
				return SCHED_STROOPER_FAIL;
			}
		}

		return SCHED_STROOPER_VICTORY_DANCE;
	}
	case SCHED_STROOPER_SUPPRESS:
	{
		if (GetEnemy()->IsPlayer() && m_fFirstEncounter)
		{
			m_fFirstEncounter = FALSE;// after first encounter, leader won't issue handsigns anymore when he has a new enemy
			return SCHED_STROOPER_SIGNAL_SUPPRESS;
		}
		else
		{
			return SCHED_STROOPER_SUPPRESS;
	...
	case SCHED_FAIL:
	{
		if (GetEnemy() != NULL)
		{
			...
			return SCHED_STROOPER_COMBAT_FAIL;
		}

		return SCHED_STROOPER_FAIL;
	}
	case SCHED_STROOPER_REPEL:
	{
	...
	case SCHED_STROOPER_REPEL_ATTACK:
	{
		Vector vecVel = GetAbsVelocity();
		if (vecVel.z > -128)
		{
...
//=========================================================
// CSTrooperRepel - when triggered, spawns a monster_shocktrooper
// repelling down a line.
//=========================================================

class CNPC_STrooperRepel :public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_STrooperRepel, CAI_BaseNPC);
public:
	void Spawn(void);
	void Precache(void);
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CNPC_STrooperRepel)
DEFINE_USEFUNC(RepelUse),
//DEFINE_FIELD( m_iSpriteTexture, FIELD_INTEGER ),
END_DATADESC()

void CNPC_STrooperRepel::Spawn(void)
{
	Precache();
	SetSolid(SOLID_NONE);

	SetUse(&CNPC_STrooperRepel::RepelUse);
}

void CNPC_STrooperRepel::Precache(void)
{
	UTIL_PrecacheOther("monster_shocktrooper");
	m_iSpriteTexture = PrecacheModel("sprites/rope.vmt");
}

void CNPC_STrooperRepel::RepelUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	trace_t tr;
	UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -4096.0), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);

	CBaseEntity *pEntity = Create("monster_shocktrooper", GetAbsOrigin(), GetAbsAngles());
...
//
//------------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC(monster_shocktrooper, CNPC_STrooper)

DECLARE_ACTIVITY(ACT_STROOPER_LAUNCH_GRENADE)
DECLARE_ACTIVITY(ACT_STROOPER_TOSS_GRENADE)
DECLARE_ACTIVITY(ACT_STROOPER_MP5_STANDING);
DECLARE_ACTIVITY(ACT_STROOPER_MP5_CROUCHING);
DECLARE_ACTIVITY(ACT_STROOPER_SHOTGUN_STANDING);
DECLARE_ACTIVITY(ACT_STROOPER_SHOTGUN_CROUCHING);

DECLARE_CONDITION(COND_STROOPER_NOFIRE)

DECLARE_TASK(TASK_STROOPER_FACE_TOSS_DIR)
DECLARE_TASK(TASK_STROOPER_SPEAK_SENTENCE)
DECLARE_TASK(TASK_STROOPER_CHECK_FIRE)
...
"	Tasks"
"		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_STROOPER_ESTABLISH_LINE_OF_FIRE_RETRY"
"		TASK_GET_PATH_TO_ENEMY		0"
"		TASK_STROOPER_SPEAK_SENTENCE	0"
"		TASK_RUN_PATH				0"
"		TASK_WAIT_FOR_MOVEMENT		0"
"	"
"	Interrupts"
...
"	Tasks"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_STROOPER_TAKE_COVER_FROM_ENEMY"
...
"		TASK_FACE_ENEMY			0"
"		TASK_WAIT				1.5"
"		TASK_SET_SCHEDULE		SCHEDULE:SCHED_STROOPER_SWEEP"
"	"
"	Interrupts"
"		COND_NEW_ENEMY"
"		COND_ENEMY_DEAD"
...
"	Tasks"
"		TASK_STOP_MOVING			0"
"		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_STROOPER_TAKE_COVER_FAILED"
"		TASK_WAIT					0.2"
...
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_STROOPER_WAIT_IN_COVER"
...
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_STROOPER_TAKE_COVER_FROM_ENEMY"
"	"
"	Interrupts"
)
...
"	Tasks"
"		TASK_STOP_MOVING				0"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_STROOPER_RELOAD"
"		TASK_FIND_COVER_FROM_ENEMY		0"
"		TASK_RUN_PATH					0"
"		TASK_WAIT_FOR_MOVEMENT			0"
"		TASK_REMEMBER					MEMORY:INCOVER"
...
"	Tasks"
"		TASK_STOP_MOVING			0"
"		TASK_STROOPER_FACE_TOSS_DIR	0"
"		TASK_PLAY_SEQUENCE			ACTIVITY:ACT_RANGE_ATTACK2"
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_STROOPER_WAIT_IN_COVER" // don't run immediately after throwing grenade.
"	"
"	Interrupts"
)
...
"		COND_HEAR_COMBAT"
	...
	DEFINE_SCHEDULE
	(
		SCHED_STROOPER_FOLLOW,
	
		"	Tasks"
		"		TASK_GET_PATH_TO_PLAYER			0"
		"		TASK_RUN_PATH					0"
		"		TASK_RUN_PATH_WITHIN_DIST		200"
	...
	(
		...
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_STROOPER_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
...
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_STROOPER_BARNACLE_PULL"
""
"	Interrupts"
)
...
"		 TASK_SET_SCHEDULE			SCHEDULE:SCHED_STROOPER_BARNACLE_CHEW"
""
"	Interrupts"
)
