...
// $NoKeywords: $
//=============================================================================//
// Repurposed into Zappas gang for TRB.
...
ConVar	sk_islave_ally_health( "sk_islave_ally_health","100");
ConVar	sk_islave_ally_dmg_claw( "sk_islave_ally_dmg_claw","10");
ConVar	sk_islave_ally_dmg_clawrake( "sk_islave_ally_dmg_clawrake","25");
ConVar	sk_islave_ally_dmg_zap( "sk_islave_ally_dmg_zap","14");

LINK_ENTITY_TO_CLASS( monster_alien_slave_ally, CNPC_HL1Allygaunt );

BEGIN_DATADESC( CNPC_HL1Allygaunt )
	DEFINE_FIELD( m_iBravery, FIELD_INTEGER ),
	DEFINE_ARRAY( m_pBeam, FIELD_CLASSPTR, ALLYGAUNT_MAX_BEAMS ),
...
enum
{
	SCHED_ALLYGAUNT_ATTACK = LAST_SHARED_SCHEDULE,
};

#define ALLYGAUNT_IGNORE_PLAYER 64

//=========================================================
// Spawn
//=========================================================
void CNPC_HL1Allygaunt::Spawn()
{

	char *szModel = (char *)STRING( GetModelName() );
	if (!szModel || !*szModel)
	{
		szModel = "models/islave_ally.mdl";
	...
	//SetModel( "models/islave_ally.mdl" );
	...
	ClearEffects();
    m_iHealth			= sk_islave_ally_health.GetFloat();
	...
	CapabilitiesClear();
	CapabilitiesAdd( bits_CAP_MOVE_GROUND | bits_CAP_MOVE_JUMP | bits_CAP_OPEN_DOORS | bits_CAP_AUTO_DOORS | bits_CAP_USE | bits_CAP_DOORS_GROUP);
	CapabilitiesAdd( bits_CAP_INNATE_RANGE_ATTACK1 | bits_CAP_INNATE_MELEE_ATTACK1 | bits_CAP_TURN_HEAD | bits_CAP_ANIMATEDFACE );
	CapabilitiesAdd( bits_CAP_SQUAD | bits_CAP_NO_HIT_SQUADMATES | bits_CAP_NO_HIT_PLAYER | bits_CAP_FRIENDLY_DMG_IMMUNE  );

	m_iBravery = 0;

	NPCInit();
	SetUse( &CNPC_HL1Allygaunt::FollowerUse );
...
//=========================================================
void CNPC_HL1Allygaunt::Precache()
{
...
//	PrecacheModel("models/islave_ally.mdl");
...
// Output :
//-----------------------------------------------------------------------------
Class_T	CNPC_HL1Allygaunt::Classify ( void )
{
	return	CLASS_CONSCRIPT;
}

void CNPC_HL1Allygaunt::CallForHelp( char *szClassname, float flDist, CBaseEntity * pEnemy, Vector &vecLocation )
{
...
// ALertSound - scream
//=========================================================
void CNPC_HL1Allygaunt::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
		...
		CallForHelp( "monster_alien_slave_ally", 3000, GetEnemy(), vecTmp );
	}
}
...
// IdleSound
//=========================================================
void CNPC_HL1Allygaunt::IdleSound( void )
{
...
// PainSound
//=========================================================
void CNPC_HL1Allygaunt::PainSound( const CTakeDamageInfo &info )
{
	if ( random->RandomInt( 0, 2 ) == 0)
	{
...
void CNPC_HL1Allygaunt::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );
	CSoundParameters params;
...
int CNPC_HL1Allygaunt::GetSoundInterests ( void )
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_DANGER	|
...
void CNPC_HL1Allygaunt::Event_Killed( const CTakeDamageInfo &info )
{
	ClearBeams( );
...
//=========================================================
float CNPC_HL1Allygaunt::MaxYawSpeed ( void )
{
	float flYS;
...
//=========================================================
void CNPC_HL1Allygaunt::HandleAnimEvent( animevent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		{
			// SOUND HERE!
			CBaseEntity *pHurt = CheckTraceHullAttack( 40, Vector(-10,-10,-10), Vector(10,10,10), sk_islave_ally_dmg_claw.GetFloat(), DMG_SLASH );
			CPASAttenuationFilter filter( this );
			if ( pHurt )
			{
		...
		{
			CBaseEntity *pHurt = CheckTraceHullAttack( 40, Vector(-10,-10,-10), Vector(10,10,10), sk_islave_ally_dmg_clawrake.GetFloat(), DMG_SLASH );
...
//			m_nSkin = m_iBeams / 2;
		...
		case ISLAVE_AE_ZAP_SHOOT:
		{
			...
			if ( m_hDead != NULL )
			{
				...
				if ( !trace.startsolid )
				{
					CBaseEntity *pNew = Create( "monster_alien_slave_ally", m_hDead->GetAbsOrigin(), m_hDead->GetAbsAngles() );
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_HL1Allygaunt::RangeAttack1Conditions( float flDot, float flDist )
{
	if ( GetEnemy() == NULL )
...
void CNPC_HL1Allygaunt::StartTask( const Task_t *pTask )
{
	ClearBeams();
...
int CNPC_HL1Allygaunt::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
void CNPC_HL1Allygaunt::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
int CNPC_HL1Allygaunt::SelectSchedule( void )
{
...
void CNPC_HL1Allygaunt::DeclineFollowing( void )
{
...
int CNPC_HL1Allygaunt::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_ALLYGAUNT_FACE_TARGET;
			else
				return baseType;
		}
		break;
	...
	case SCHED_IDLE_STAND:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_ALLYGAUNT_IDLE_STAND;
			else
				return baseType;
		}
		break;
...
//=========================================================
void CNPC_HL1Allygaunt::ArmBeam( int side )
{
	trace_t tr;
	float flDist = 1.0;
	
	if ( m_iBeams >= ALLYGAUNT_MAX_BEAMS )
		 return;
...
//=========================================================
void CNPC_HL1Allygaunt::BeamGlow( )
{
	int b = m_iBeams * 32;
...
//=========================================================
void CNPC_HL1Allygaunt::WackBeam( int side, CBaseEntity *pEntity )
{
	Vector vecDest;
...
//=========================================================
void CNPC_HL1Allygaunt::ZapBeam( int side )
{
	...
	{
		CTakeDamageInfo info( this, this, sk_islave_ally_dmg_zap.GetFloat(), DMG_SHOCK );
		CalculateMeleeDamageForce( &info, vecAim, tr.endpos );
...
//=========================================================
void CNPC_HL1Allygaunt::ClearBeams( )
{
	for (int i = 0; i < ALLYGAUNT_MAX_BEAMS; i++)
	{
		if (m_pBeam[i])
		{
...
AI_BEGIN_CUSTOM_NPC( monster_alien_slave_ally, CNPC_HL1Allygaunt )

	//=========================================================
	// > SCHED_ALLYGAUNT_ATTACK
	//=========================================================
	DEFINE_SCHEDULE
	(
		SCHED_ALLYGAUNT_ATTACK,
...
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_BARNEY_STOP_FOLLOWING"
	...
	(
		...
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_ALLYGAUNT_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
