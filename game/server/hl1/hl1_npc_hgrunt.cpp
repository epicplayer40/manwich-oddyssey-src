...
ConVar	sk_hgrunt_health("sk_hgrunt_health", "125");
ConVar  sk_hgrunt_kick("sk_hgrunt_kick", "10");
ConVar  sk_hgrunt_pellets("sk_hgrunt_pellets", "7");
ConVar  sk_hgrunt_gspeed("sk_hgrunt_gspeed", "600");
...
#define	HGRUNT_SENTENCE_VOLUME			(float)1.0 // volume of grunt sentences
...
//	else
	...
	SetUse( &CNPC_HGrunt::FollowerUse );
}



void CNPC_HGrunt::FollowerUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
//		}
	...
	CNPC_HGrunt::FollowerUse( pActivator, pCaller, useType, value );
}
...
void CNPC_HGrunt::Precache()
{
	...
	PrecacheModel("models/hgrunt.mdl");//red guy

	PrecacheModel("models/hgrunt01.mdl");//red guy
	PrecacheModel("models/hgrunt02.mdl");//gasmask citizen guy
	PrecacheModel("models/hgrunt03.mdl");//brown guy
	PrecacheModel("models/hgrunt04.mdl");//blue guy
	PrecacheModel("models/hgrunt05.mdl");//green guy
...
//	}
	...
	CNPC_HGrunt::StartFollowing( pLeader );
}

void CNPC_HGrunt::StopFollowing( void )
{

	CNPC_HGrunt::StopFollowing();
}
...
void CNPC_HGrunt::IdleSound(void)
{
	if (FOkToSpeak() && (g_fGruntQuestion || random->RandomInt(0, 1)))
	{
		if (!g_fGruntQuestion)
		{
			...
			{
			case 0: // check in
				SENTENCEG_PlayRndSz(edict(), "HG_CHECK", HGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				g_fGruntQuestion = 1;
				break;
			case 1: // question
				SENTENCEG_PlayRndSz(edict(), "HG_QUEST", HGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				g_fGruntQuestion = 2;
				break;
			case 2: // statement
				SENTENCEG_PlayRndSz(edict(), "HG_IDLE", HGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			}
		}
		else
		{
			...
			{
			case 1: // check in
				SENTENCEG_PlayRndSz(edict(), "HG_CLEAR", HGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			case 2: // question 
				SENTENCEG_PlayRndSz(edict(), "HG_ANSWER", HGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			}
			g_fGruntQuestion = 0;
		}
	...
	case NPC_STATE_COMBAT:
	{
		...
		else if (HasCondition(COND_LIGHT_DAMAGE))
		{
			...
			if (iPercent <= 90 && GetEnemy() != NULL)
			{
				...
				if (FOkToSpeak()) // && RANDOM_LONG(0,1))
				{
					SENTENCEG_PlayRndSz(edict(), "HG_COVER", HGRUNT_SENTENCE_VOLUME, GRUNT_SNDLVL, 0, m_voicePitch);
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_HGRUNT_FACE_TARGET;
			else
				return baseType;
		}
		break;
...
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_BARNEY_STOP_FOLLOWING"
	...
	DEFINE_SCHEDULE
	(
		SCHED_HGRUNT_FACE_TARGET,
	
		"	Tasks"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_HGRUNT_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
