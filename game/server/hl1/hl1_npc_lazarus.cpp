...
//4: Make a version with its own CLASS_ faction for each of the 5 variants for after the militia breaks off, monster_lazarus_team1 with CLASS_TEAM1, monster_lazarus_team2 with CLASS_TEAM2, etc. (Haven't started yet, it'll be tedious)
...
ConVar	sk_lazarus_moses_health( "sk_lazarus_moses_health","500");

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
// first flag is lazarus dying for scripted sequences?
#define		LAZARUS_AE_DRAW		( 2 )
#define		LAZARUS_AE_SHOOT		( 3 )
#define		LAZARUS_AE_HOLSTER	( 4 )

#define		LAZARUS_BODY_GUNHOLSTERED	0
#define		LAZARUS_BODY_GUNDRAWN		1
#define		LAZARUS_BODY_GUNGONE			2


enum LazarusSquadSlot_T
{	
	LAZARUS_SQUAD_SLOT_ATTACK1 = LAST_SHARED_SQUADSLOT,
	LAZARUS_SQUAD_SLOT_ATTACK2,
	LAZARUS_SQUAD_SLOT_INVESTIGATE_SOUND,
	LAZARUS_SQUAD_SLOT_CHASE,
};
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_HL1Lazarus )
...
LINK_ENTITY_TO_CLASS( monster_lazarus, CNPC_HL1Lazarus );
...
void CNPC_HL1Lazarus::Spawn()
{
	...
	// Select random model
	SetModel( "models/squad_lazarusmoses.mdl" );
	...
	m_iHealth			= sk_lazarus_moses_health.GetFloat();
	...
	SetUse( &CNPC_HL1Lazarus::FollowerUse );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CNPC_HL1Lazarus::Precache()
{
	m_iAmmoType = GetAmmoDef()->Index("357");
	PrecacheModel( STRING( GetModelName() ) );

	PrecacheModel( "models/squad_lazarusmoses.mdl" );

	PrecacheScriptSound( "Lazarus.FireWeapon" );
	PrecacheScriptSound( "Lazarus.Pain" );
	PrecacheScriptSound( "Lazarus.Die" );

	// every new lazarus must call this, otherwise
	// when a level is loaded, nobody will talk (time is reset to 0)
	TalkInit();
	BaseClass::Precache();
}	

void CNPC_HL1Lazarus::ModifyOrAppendCriteria( AI_CriteriaSet& criteriaSet )
{
...
// Init talk data
void CNPC_HL1Lazarus::TalkInit()
{
	BaseClass::TalkInit();

	// get voice for head - just one lazarus voice for now
...
//=========================================================
int CNPC_HL1Lazarus::GetSoundInterests ( void) 
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_CARCASS	|
...
// relationship table.
//=========================================================
Class_T	CNPC_HL1Lazarus::Classify ( void )
{
	return	CLASS_CONSCRIPT;
}

//=========================================================
// ALertSound - lazarus says "Freeze!"
//=========================================================
void CNPC_HL1Lazarus::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
	//	if ( IsOkToSpeak() )
...
//=========================================================
void CNPC_HL1Lazarus::SetYawSpeed ( void )
{
	int ys;
...
// CheckRangeAttack1
//=========================================================
bool CNPC_HL1Lazarus::CheckRangeAttack1 ( float flDot, float flDist )
{
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_HL1Lazarus::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
	...
	else if ( flDist > 4000 ) //1024 originally
	{
...
//=========================================================
// LazarusFirePistol - shoots one round from the pistol at
// the enemy lazarus is facing.
//=========================================================
void CNPC_HL1Lazarus::LazarusFirePistol ( void )
{
...
//	SetBlending( 0, angDir.x );
	...
	FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_1DEGREES, 4000, m_iAmmoType ); //raised gun range to 1200 in accordance with range increase
	...
	EmitSound_t params;
	params.m_pSoundName = "Lazarus.FireWeapon";
	params.m_flVolume = 1;
...
int CNPC_HL1Lazarus::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
// PainSound
//=========================================================
void CNPC_HL1Lazarus::PainSound( const CTakeDamageInfo &info )
{
	...
	if ( GetParametersForSound( "Lazarus.Pain", params, NULL ) )
	{
...
// DeathSound 
//=========================================================
void CNPC_HL1Lazarus::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );

	CSoundParameters params;
	if ( GetParametersForSound( "Lazarus.Die", params, NULL ) )
	{
		params.pitch = GetExpresser()->GetVoicePitch();
...
void CNPC_HL1Lazarus::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_HL1Lazarus::Event_Killed( const CTakeDamageInfo &info )
{
	if ( m_nBody < LAZARUS_BODY_GUNGONE )
	{
		// drop the gun!
		Vector vecGunPos;
		QAngle angGunAngles;
		...
		SetBodygroup( 1, LAZARUS_BODY_GUNGONE);

		GetAttachment( "0", vecGunPos, angGunAngles );
		
		angGunAngles.y += 180;
		pGun = DropItem( "weapon_annabelle", vecGunPos, angGunAngles );
	}
...
void CNPC_HL1Lazarus::SUB_StartLVFadeOut( float delay, bool notSolid )
{
	SetThink( &CNPC_HL1Lazarus::SUB_LVFadeOut );
...
void CNPC_HL1Lazarus::SUB_LVFadeOut( void  )
{
...
void CNPC_HL1Lazarus::StartTask( const Task_t *pTask )
{
	BaseClass::StartTask( pTask );	
}

void CNPC_HL1Lazarus::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//=========================================================
void CNPC_HL1Lazarus::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
	case LAZARUS_AE_SHOOT:
		LazarusFirePistol();
		break;

	case LAZARUS_AE_DRAW:
		// lazarus's bodygroup switches here so he can pull gun from holster
		SetBodygroup( 1, LAZARUS_BODY_GUNDRAWN);
		m_fGunDrawn = true;
		break;

	case LAZARUS_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		SetBodygroup( 1, LAZARUS_BODY_GUNHOLSTERED);
		m_fGunDrawn = false;
		break;
...
int CNPC_HL1Lazarus::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	case SCHED_ARM_WEAPON:
		if ( GetEnemy() != NULL )
		{
			...
			return SCHED_LAZARUS_ENEMY_DRAW;
		}
		break;
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_LAZARUS_FACE_TARGET;
			else
				return baseType;
		}
		break;
	...
	case SCHED_IDLE_STAND:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_LAZARUS_IDLE_STAND;
			else
				return baseType;
		}
		break;
...
// of the proper type.
//=========================================================
int CNPC_HL1Lazarus::SelectSchedule( void )
{
...
NPC_STATE CNPC_HL1Lazarus::SelectIdealState ( void )
{
	return BaseClass::SelectIdealState();
}

void CNPC_HL1Lazarus::DeclineFollowing( void )
{
...
bool CNPC_HL1Lazarus::CanBecomeRagdoll( void )
{
	if ( UTIL_IsLowViolence() )
	{
		return false;
...
bool CNPC_HL1Lazarus::ShouldGib( const CTakeDamageInfo &info )
{
//	if ( UTIL_IsLowViolence() )
//	{
//		return false;
...
bool CNPC_HL1Lazarus::CorpseGib( const CTakeDamageInfo &info )
{
	CEffectData	data;
...
AI_BEGIN_CUSTOM_NPC( monster_lazarus, CNPC_HL1Lazarus )

	DECLARE_SQUADSLOT( LAZARUS_SQUAD_SLOT_ATTACK1 )
	DECLARE_SQUADSLOT( LAZARUS_SQUAD_SLOT_ATTACK2 )
	DECLARE_SQUADSLOT( LAZARUS_SQUAD_SLOT_CHASE )
	DECLARE_SQUADSLOT( LAZARUS_SQUAD_SLOT_INVESTIGATE_SOUND )

	//=========================================================
	// > SCHED_LAZARUS_FOLLOW
	//=========================================================
	DEFINE_SCHEDULE
	(
		SCHED_LAZARUS_FOLLOW,
		...
		"	Tasks"
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_LAZARUS_STOP_FOLLOWING"
	...
	(
		...
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_LAZARUS_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
...
//
//=========================================================
class CNPC_DeadLazarus : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_DeadLazarus, CAI_BaseNPC );
public:
...
char *CNPC_DeadLazarus::m_szPoses[] = { "lying_on_back", "lying_on_side", "lying_on_stomach" };

bool CNPC_DeadLazarus::KeyValue( const char *szKeyName, const char *szValue )
{
...
LINK_ENTITY_TO_CLASS( monster_lazarus_dead, CNPC_DeadLazarus );

BEGIN_DATADESC( CNPC_DeadLazarus )
END_DATADESC()

//=========================================================
// ********** DeadLazarus SPAWN **********
//=========================================================
void CNPC_DeadLazarus::Spawn( void )
{
	PrecacheModel("models/lazarushl1.mdl");
	SetModel( "models/lazarushl1.mdl");
	...
	if ( GetSequence() == -1 )
	{
		Msg ( "Dead lazarus with bad pose\n" );
	}
	// Corpses have less health
	m_iHealth			= 0.0;//gSkillData.lazarusHealth;
