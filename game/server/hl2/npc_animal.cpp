...
//
// Purpose: A rambling, jumpy conscript who will meet his doom at the hands of a sniper if the player doesn't cover him.
// Not a vital npc but if you keep him alive the sniper chapter will be a lot easier.
//
//=============================================================================//
...
#define ANIMAL_MODEL "models/animal.mdl"

ConVar	sk_animal_health( "sk_animal_health","50");

//=========================================================
// Barney activities
//=========================================================

class CNPC_Animal : public CNPC_PlayerCompanion
{
public:
	DECLARE_CLASS( CNPC_Animal, CNPC_PlayerCompanion );
	DECLARE_SERVERCLASS();
	DECLARE_DATADESC();
...
LINK_ENTITY_TO_CLASS( npc_animal, CNPC_Animal );

//---------------------------------------------------------
// 
//---------------------------------------------------------
//IMPLEMENT_CUSTOM_AI( npc_conscript,CNPC_Animal );
IMPLEMENT_SERVERCLASS_ST(CNPC_Animal, DT_NPC_Animal)
END_SEND_TABLE()
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Animal::SelectModel()
{
	SetModelName( AllocPooledString( ANIMAL_MODEL ) );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Animal::Spawn( void )
{
	Precache();

	m_iHealth = sk_animal_health.GetFloat();;;
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CNPC_Animal::Classify( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Animal::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Animal::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CNPC_Animal::DeathSound( const CTakeDamageInfo &info )
{
...
bool CNPC_Animal::CreateBehaviors( void )
{
...
void CNPC_Animal::OnChangeRunningBehavior( CAI_BehaviorBase *pOldBehavior,  CAI_BehaviorBase *pNewBehavior )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Animal::GatherConditions()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Animal::UseFunc( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
AI_BEGIN_CUSTOM_NPC( npc_conscript, CNPC_Animal )
