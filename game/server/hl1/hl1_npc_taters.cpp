...
//4: Make a version with its own CLASS_ faction for each of the 5 variants for after the militia breaks off, monster_taters_team1 with CLASS_TEAM1, monster_taters_team2 with CLASS_TEAM2, etc. (Haven't started yet, it'll be tedious)
...
ConVar	sk_tatershl1_health( "sk_tatershl1_health","500");

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
// first flag is taters dying for scripted sequences?
#define		TATERS_AE_DRAW		( 2 )
#define		TATERS_AE_SHOOT		( 3 )
#define		TATERS_AE_HOLSTER	( 4 )

#define		TATERS_BODY_GUNHOLSTERED	0
#define		TATERS_BODY_GUNDRAWN		1
#define		TATERS_BODY_GUNGONE			2


enum TatersSquadSlot_T
{	
	TATERS_SQUAD_SLOT_ATTACK1 = LAST_SHARED_SQUADSLOT,
	TATERS_SQUAD_SLOT_ATTACK2,
	TATERS_SQUAD_SLOT_INVESTIGATE_SOUND,
	TATERS_SQUAD_SLOT_CHASE,
};
...
LINK_ENTITY_TO_CLASS( monster_taters, CNPC_HL1Taters );
...
// Spawn
//=========================================================
void CNPC_HL1Taters::Spawn()
{
	Precache( );

	SetModel( "models/squad_taters.mdl");
	...
	m_iHealth			= sk_tatershl1_health.GetFloat();
	...
	SetUse( &CNPC_HL1Taters::FollowerUse );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CNPC_HL1Taters::Precache()
{
	m_iAmmoType = GetAmmoDef()->Index("Pistol");
	PrecacheModel( STRING( GetModelName() ) );

	PrecacheModel( "models/squad_taters.mdl" );

	PrecacheScriptSound( "Taters.FireWeapon" );
	PrecacheScriptSound( "Taters.Pain" );
	PrecacheScriptSound( "Taters.Die" );

	// every new taters must call this, otherwise
	// when a level is loaded, nobody will talk (time is reset to 0)
	TalkInit();
	BaseClass::Precache();
}	

void CNPC_HL1Taters::ModifyOrAppendCriteria( AI_CriteriaSet& criteriaSet )
{
...
// Init talk data
void CNPC_HL1Taters::TalkInit()
{
	BaseClass::TalkInit();

	// get voice for head - just one taters voice for now
...
//=========================================================
int CNPC_HL1Taters::GetSoundInterests ( void) 
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_CARCASS	|
...
// relationship table.
//=========================================================
Class_T	CNPC_HL1Taters::Classify ( void )
{
	return	CLASS_CONSCRIPT;
}

//=========================================================
// ALertSound - taters says "Freeze!"
//=========================================================
void CNPC_HL1Taters::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
	//	if ( IsOkToSpeak() )
...
//=========================================================
void CNPC_HL1Taters::SetYawSpeed ( void )
{
	int ys;
...
// CheckRangeAttack1
//=========================================================
bool CNPC_HL1Taters::CheckRangeAttack1 ( float flDot, float flDist )
{
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_HL1Taters::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
//=========================================================
// TatersFirePistol - shoots one round from the pistol at
// the enemy taters is facing.
//=========================================================
void CNPC_HL1Taters::TatersFirePistol ( void )
{
...
//	SetBlending( 0, angDir.x );
	...
	FireBullets(8, vecShootOrigin, vecShootDir, VECTOR_CONE_10DEGREES, 1024, m_iAmmoType ); //raised gun range to 1200 in accordance with range increase
	...
	EmitSound_t params;
	params.m_pSoundName = "Taters.FireWeapon";
	params.m_flVolume = 1;
...
int CNPC_HL1Taters::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
// PainSound
//=========================================================
void CNPC_HL1Taters::PainSound( const CTakeDamageInfo &info )
{
	...
	if ( GetParametersForSound( "Taters.Pain", params, NULL ) )
	{
...
// DeathSound 
//=========================================================
void CNPC_HL1Taters::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );

	CSoundParameters params;
	if ( GetParametersForSound( "Taters.Die", params, NULL ) )
	{
		params.pitch = GetExpresser()->GetVoicePitch();
...
void CNPC_HL1Taters::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_HL1Taters::Event_Killed( const CTakeDamageInfo &info )
{
	if ( m_nBody < TATERS_BODY_GUNGONE )
	{
		// drop the gun!
		Vector vecGunPos;
		QAngle angGunAngles;
		...
		SetBodygroup( 1, TATERS_BODY_GUNGONE);
...
void CNPC_HL1Taters::SUB_StartLVFadeOut( float delay, bool notSolid )
{
	SetThink( &CNPC_HL1Taters::SUB_LVFadeOut );
...
void CNPC_HL1Taters::SUB_LVFadeOut( void  )
{
...
void CNPC_HL1Taters::StartTask( const Task_t *pTask )
{
	BaseClass::StartTask( pTask );	
}

void CNPC_HL1Taters::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//=========================================================
void CNPC_HL1Taters::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
	case TATERS_AE_SHOOT:
		TatersFirePistol();
		break;

	case TATERS_AE_DRAW:
		// taters's bodygroup switches here so he can pull gun from holster
		SetBodygroup( 1, TATERS_BODY_GUNDRAWN);
		m_fGunDrawn = true;
		break;

	case TATERS_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		SetBodygroup( 1, TATERS_BODY_GUNHOLSTERED);
		m_fGunDrawn = false;
		break;
...
int CNPC_HL1Taters::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
	case SCHED_ARM_WEAPON:
		if ( GetEnemy() != NULL )
		{
			...
			return SCHED_TATERS_ENEMY_DRAW;
		}
		break;
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_TATERS_FACE_TARGET;
			else
				return baseType;
		}
		break;
	...
	case SCHED_IDLE_STAND:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_TATERS_IDLE_STAND;
			else
				return baseType;
		}
		break;
...
// of the proper type.
//=========================================================
int CNPC_HL1Taters::SelectSchedule( void )
{
...
NPC_STATE CNPC_HL1Taters::SelectIdealState ( void )
{
	return BaseClass::SelectIdealState();
}

void CNPC_HL1Taters::DeclineFollowing( void )
{
...
bool CNPC_HL1Taters::CanBecomeRagdoll( void )
{
	if ( UTIL_IsLowViolence() )
	{
		return false;
...
bool CNPC_HL1Taters::ShouldGib( const CTakeDamageInfo &info )
{
//	if ( UTIL_IsLowViolence() )
//	{
//		return false;
...
bool CNPC_HL1Taters::CorpseGib( const CTakeDamageInfo &info )
{
	CEffectData	data;
...
AI_BEGIN_CUSTOM_NPC( monster_taters, CNPC_HL1Taters )

	DECLARE_SQUADSLOT( TATERS_SQUAD_SLOT_ATTACK1 )
	DECLARE_SQUADSLOT( TATERS_SQUAD_SLOT_ATTACK2 )
	DECLARE_SQUADSLOT( TATERS_SQUAD_SLOT_CHASE )
	DECLARE_SQUADSLOT( TATERS_SQUAD_SLOT_INVESTIGATE_SOUND )
	...
	(
		...
		"	Tasks"
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_TATERS_STOP_FOLLOWING"
	...
	(
		...
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_TATERS_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
...
//
//=========================================================
class CNPC_DeadTaters : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_DeadTaters, CAI_BaseNPC );
public:
...
char *CNPC_DeadTaters::m_szPoses[] = { "lying_on_back", "lying_on_side", "lying_on_stomach" };

bool CNPC_DeadTaters::KeyValue( const char *szKeyName, const char *szValue )
{
...
LINK_ENTITY_TO_CLASS( monster_taters_dead, CNPC_DeadTaters );

BEGIN_DATADESC( CNPC_DeadTaters )
END_DATADESC()

//=========================================================
// ********** DeadTaters SPAWN **********
//=========================================================
void CNPC_DeadTaters::Spawn( void )
{
	PrecacheModel("models/tatershl1.mdl");
	SetModel( "models/tatershl1.mdl");
	...
	if ( GetSequence() == -1 )
	{
		Msg ( "Dead taters with bad pose\n" );
	}
	// Corpses have less health
	m_iHealth			= 0.0;//gSkillData.tatersHealth;
