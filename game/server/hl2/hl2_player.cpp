...
void CHL2_Player::PostThink( void )
{
	...
	if (GetHealth()<=10)
	{
            ...
            trace_t trfloor;
            UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -16), MASK_PLAYERSOLID, this, COLLISION_GROUP_NONE, &trfloor);

            if (trfloor.DidHitWorld()) {

                if (GetAbsVelocity().Length()!=0.0f){
                    UTIL_DecalTrace(&trfloor, "Bloodstain_02");
					//m_flLastBleedTime = gpGlobals->curtime + 6.0f;
				}
			}
...
{
#ifndef HL2MP	
	if ( GlobalEntity_GetState("stunsticks_usable") != GLOBAL_ON && pWeapon->ClassMatches("weapon_stunstick"))
	{
		if ( ApplyBattery( 0.5 ) )
			UTIL_Remove( pWeapon );
		return false;
