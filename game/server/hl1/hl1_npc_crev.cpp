...
//4: Make a version with its own CLASS_ faction for each of the 5 variants for after the militia breaks off, monster_crev_team1 with CLASS_TEAM1, monster_crev_team2 with CLASS_TEAM2, etc. (Haven't started yet, it'll be tedious)
...
#define CV_ATTACK	"CV_ATTACK"
#define CV_MAD		"CV_MAD"
#define CV_SHOT		"CV_SHOT"
#define CV_KILL		"CV_KILL"
#define CV_POK		"CV_POK"

ConVar	sk_crevhl1_health( "sk_crevhl1_health","500");

//=========================================================
// Monster's Anim Events Go Here
//=========================================================
// first flag is crev dying for scripted sequences?
#define		CREV_AE_DRAW		( 2 )
#define		CREV_AE_SHOOT		( 3 )
#define		CREV_AE_HOLSTER	( 4 )

#define		CREV_BODY_GUNHOLSTERED	0
#define		CREV_BODY_GUNDRAWN		1
#define		CREV_BODY_GUNGONE			2


enum CrevSquadSlot_T
{	
	CREV_SQUAD_SLOT_ATTACK1 = LAST_SHARED_SQUADSLOT,
	CREV_SQUAD_SLOT_ATTACK2,
	CREV_SQUAD_SLOT_INVESTIGATE_SOUND,
	CREV_SQUAD_SLOT_CHASE,
};
...
LINK_ENTITY_TO_CLASS( monster_crev, CNPC_HL1Crev );
...
// Spawn
//=========================================================
void CNPC_HL1Crev::Spawn()
{
	Precache( );

	SetModel( "models/squad_crev.mdl");
	...
	m_iHealth			= sk_crevhl1_health.GetFloat();
	...
	SetUse( &CNPC_HL1Crev::FollowerUse );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CNPC_HL1Crev::Precache()
{
	m_iAmmoType = GetAmmoDef()->Index("Pistol");
	PrecacheModel( STRING( GetModelName() ) );

	PrecacheModel( "models/squad_crev.mdl" );

	PrecacheScriptSound( "Crev.FireWeapon" );
	PrecacheScriptSound( "Crev.Pain" );
	PrecacheScriptSound( "Crev.Die" );

	// every new crev must call this, otherwise
	// when a level is loaded, nobody will talk (time is reset to 0)
	TalkInit();
	BaseClass::Precache();
}	

void CNPC_HL1Crev::ModifyOrAppendCriteria( AI_CriteriaSet& criteriaSet )
{
...
// Init talk data
void CNPC_HL1Crev::TalkInit()
{
	BaseClass::TalkInit();

	// get voice for head - just one crev voice for now
...
//=========================================================
int CNPC_HL1Crev::GetSoundInterests ( void) 
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_CARCASS	|
...
// relationship table.
//=========================================================
Class_T	CNPC_HL1Crev::Classify ( void )
{
	return	CLASS_CONSCRIPT;
}

//=========================================================
// ALertSound - crev says "Freeze!"
//=========================================================
void CNPC_HL1Crev::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
	//	if ( IsOkToSpeak() )
...
//=========================================================
void CNPC_HL1Crev::SetYawSpeed ( void )
{
	int ys;
...
// CheckRangeAttack1
//=========================================================
bool CNPC_HL1Crev::CheckRangeAttack1 ( float flDot, float flDist )
{
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_HL1Crev::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
//=========================================================
// CrevFirePistol - shoots one round from the pistol at
// the enemy crev is facing.
//=========================================================
void CNPC_HL1Crev::CrevFirePistol ( void )
{
...
//	SetBlending( 0, angDir.x );
	...
	EmitSound_t params;
	params.m_pSoundName = "Crev.FireWeapon";
	params.m_flVolume = 1;
...
int CNPC_HL1Crev::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
// PainSound
//=========================================================
void CNPC_HL1Crev::PainSound( const CTakeDamageInfo &info )
{
	...
	if ( GetParametersForSound( "Crev.Pain", params, NULL ) )
	{
...
// DeathSound 
//=========================================================
void CNPC_HL1Crev::DeathSound( const CTakeDamageInfo &info )
{
	CPASAttenuationFilter filter( this );

	CSoundParameters params;
	if ( GetParametersForSound( "Crev.Die", params, NULL ) )
	{
		params.pitch = GetExpresser()->GetVoicePitch();
...
void CNPC_HL1Crev::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_HL1Crev::Event_Killed( const CTakeDamageInfo &info )
{
	if ( m_nBody < CREV_BODY_GUNGONE )
	{
		// drop the gun!
		Vector vecGunPos;
		QAngle angGunAngles;
		...
		SetBodygroup( 1, CREV_BODY_GUNGONE);
...
void CNPC_HL1Crev::SUB_StartLVFadeOut( float delay, bool notSolid )
{
	SetThink( &CNPC_HL1Crev::SUB_LVFadeOut );
...
void CNPC_HL1Crev::SUB_LVFadeOut( void  )
{
...
void CNPC_HL1Crev::StartTask( const Task_t *pTask )
{
	BaseClass::StartTask( pTask );	
}

void CNPC_HL1Crev::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//=========================================================
void CNPC_HL1Crev::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
	case CREV_AE_SHOOT:
	...
	case CREV_AE_DRAW:
		// crev's bodygroup switches here so he can pull gun from holster
		SetBodygroup( 1, CREV_BODY_GUNDRAWN);
		m_fGunDrawn = true;
		break;

	case CREV_AE_HOLSTER:
		// change bodygroup to replace gun in holster
		SetBodygroup( 1, CREV_BODY_GUNHOLSTERED);
		m_fGunDrawn = false;
		break;
...
int CNPC_HL1Crev::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
// of the proper type.
//=========================================================
int CNPC_HL1Crev::SelectSchedule( void )
{
...
void CNPC_HL1Crev::DeclineFollowing( void )
{
...
bool CNPC_HL1Crev::CanBecomeRagdoll( void )
{
	if ( UTIL_IsLowViolence() )
	{
		return false;
...
bool CNPC_HL1Crev::ShouldGib( const CTakeDamageInfo &info )
{
//	if ( UTIL_IsLowViolence() )
//	{
//		return false;
...
bool CNPC_HL1Crev::CorpseGib( const CTakeDamageInfo &info )
{
	CEffectData	data;
...
AI_BEGIN_CUSTOM_NPC( monster_crev, CNPC_HL1Crev )

	DECLARE_SQUADSLOT( CREV_SQUAD_SLOT_ATTACK1 )
	DECLARE_SQUADSLOT( CREV_SQUAD_SLOT_ATTACK2 )
	DECLARE_SQUADSLOT( CREV_SQUAD_SLOT_CHASE )
	DECLARE_SQUADSLOT( CREV_SQUAD_SLOT_INVESTIGATE_SOUND )
	...
	(
		...
		"	Tasks"
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CREV_STOP_FOLLOWING"
	...
	(
		...
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_CREV_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
...
//
//=========================================================
class CNPC_DeadCrev : public CAI_BaseNPC
{
	DECLARE_CLASS( CNPC_DeadCrev, CAI_BaseNPC );
public:
...
char *CNPC_DeadCrev::m_szPoses[] = { "lying_on_back", "lying_on_side", "lying_on_stomach" };

bool CNPC_DeadCrev::KeyValue( const char *szKeyName, const char *szValue )
{
...
LINK_ENTITY_TO_CLASS( monster_crev_dead, CNPC_DeadCrev );

BEGIN_DATADESC( CNPC_DeadCrev )
END_DATADESC()

//=========================================================
// ********** DeadCrev SPAWN **********
//=========================================================
void CNPC_DeadCrev::Spawn( void )
{
	PrecacheModel("models/crevhl1.mdl");
	SetModel( "models/crevhl1.mdl");
	...
	if ( GetSequence() == -1 )
	{
		Msg ( "Dead crev with bad pose\n" );
	}
	// Corpses have less health
	m_iHealth			= 0.0;//gSkillData.crevHealth;
