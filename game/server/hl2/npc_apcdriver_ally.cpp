...
#define SF_APCDRIVERALLY_NO_ROCKET_ATTACK	0x10000
#define SF_APCDRIVERALLY_NO_GUN_ATTACK		0x20000

#define NPC_APCDRIVERALLY_REMEMBER_TIME		4


//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
class CNPC_APCDriverAlly : public CNPC_VehicleDriver
{
	DECLARE_CLASS( CNPC_APCDriverAlly, CNPC_VehicleDriver );
public:
	DECLARE_DATADESC();
	DEFINE_CUSTOM_AI;
...
BEGIN_DATADESC( CNPC_APCDriverAlly )
...
LINK_ENTITY_TO_CLASS( npc_apcdriverally, CNPC_APCDriverAlly );


//------------------------------------------------------------------------------
// Purpose :
//------------------------------------------------------------------------------
void CNPC_APCDriverAlly::Spawn( void )
{
	BaseClass::Spawn();

	m_flTimeLastSeenEnemy = -NPC_APCDRIVERALLY_REMEMBER_TIME;
	CapabilitiesClear();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_APCDriverAlly::Activate( void )
{
	BaseClass::Activate();
	...
	if ( !m_hAPC )
	{
		Warning( "npc_apcdriverally %s couldn't find his apc named %s.\n", STRING(GetEntityName()), STRING(m_iszVehicleName) );
		UTIL_Remove( this );
		return;
	}
	SetParent( m_hAPC );
...
// Enable, disable firing
//-----------------------------------------------------------------------------
void CNPC_APCDriverAlly::InputEnableFiring( inputdata_t &inputdata )
{
	m_bFiringDisabled = false;
}

void CNPC_APCDriverAlly::InputDisableFiring( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
Disposition_t CNPC_APCDriverAlly::IRelationType(CBaseEntity *pTarget)
{
...
// Are we being carried by a dropship?
//------------------------------------------------------------------------------
bool CNPC_APCDriverAlly::IsBeingCarried()
{
...
//------------------------------------------------------------------------------
bool CNPC_APCDriverAlly::WeaponLOSCondition(const Vector &ownerPos, const Vector &targetPos, bool bSetConditions)
{
...
// Is the enemy visible?
//-----------------------------------------------------------------------------
bool CNPC_APCDriverAlly::FVisible( CBaseEntity *pTarget, int traceMask, CBaseEntity **ppBlocker )
{
	if ( m_hAPC->m_lifeState != LIFE_ALIVE )
		return false;
	...
	if ( pTarget->IsPlayer() || pTarget->Classify() == CLASS_BULLSEYE )
	{
		if (!bVisible)
		{
			if ( ( gpGlobals->curtime - m_flTimeLastSeenEnemy ) <= NPC_APCDRIVERALLY_REMEMBER_TIME )
				return true;
		}
	}
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_APCDriverAlly::RangeAttack1Conditions( float flDot, float flDist )
{
	if ( HasSpawnFlags(SF_APCDRIVERALLY_NO_GUN_ATTACK) )
		return COND_NONE;
...
int CNPC_APCDriverAlly::RangeAttack2Conditions( float flDot, float flDist )
{
	if ( HasSpawnFlags(SF_APCDRIVERALLY_NO_ROCKET_ATTACK) )
		return COND_NONE;
...
// Aim the laser dot!
//-----------------------------------------------------------------------------
void CNPC_APCDriverAlly::PrescheduleThink( )
{
...
//
//-----------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC( npc_apcdriverally, CNPC_APCDriverAlly )
