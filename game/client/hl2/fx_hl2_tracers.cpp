...
void MuzzleFlash_Aids( ClientEntityHandle_t hEntity, int attachmentIndex )
{
	VPROF_BUDGET( "MuzzleFlash_Aids", VPROF_BUDGETGROUP_PARTICLE_RENDERING );
	...
	pParticle = (SimpleParticle *) pSimple->AddParticle( sizeof( SimpleParticle ), pSimple->GetPMaterial( "effects/aidsmuzzle" ), offset );
	if ( pParticle == NULL )
		return;
	...
	pParticle->m_uchColor[0]	= 100;
	pParticle->m_uchColor[1]	= 255;
	pParticle->m_uchColor[2]	= 100;
...
void AidsMuzzleFlashCallback( const CEffectData &data )
{
	MuzzleFlash_Aids( data.m_hEntity, data.m_nAttachmentIndex );
}

DECLARE_CLIENT_EFFECT( "AidsMuzzleFlash", AidsMuzzleFlashCallback );
