...
#include	"AI_Motor.h"
#include	"hl1_npc_hgrunt_combine.h"
#include	"soundent.h"
#include	"game.h"
#include	"NPCEvent.h"
...
#define SF_CGRUNT_LEADER	( 1 << 5  )

extern int g_fGruntQuestion;				// true if an idle grunt asked a question. Cleared when someone answers.
extern int g_iSquadIndex;

#define FHCGRUNT_GUN_SPREAD 0.08716f

//=========================================================
// monster-specific DEFINE's
//=========================================================
#define	CGRUNT_CLIP_SIZE					36 // how many bullets in a clip? - NOTE: 3 round burst sound, so keep as 3 * x!
#define CGRUNT_VOL						0.35		// volume of grunt sounds
#define CGRUNT_SNDLVL					SNDLVL_TALKING	// soundlevel of grunt sentences
#define FHCGRUNT_LIMP_HEALTH				sk_hgrunt_health.GetFloat() / 2 //Limp when at half health
#define FHCGRUNT_DMG_HEADSHOT				( DMG_BULLET | DMG_CLUB )	// damage types that can kill a grunt with a single headshot.
#define FHCGRUNT_NUM_HEADS				2 // how many grunt heads are there? 
#define FHCGRUNT_MINIMUM_HEADSHOT_DAMAGE	15 // must do at least this much damage in one shot to head to score a headshot kill
#define	FHCGRUNT_SENTENCE_VOLUME			(float)1.0 // volume of grunt sentences

#define FHCGRUNT_9MMAR				( 1 << 0)
#define FHCGRUNT_HANDGRENADE			( 1 << 1)
#define FHCGRUNT_GRENADELAUNCHER		( 1 << 2)
#define FHCGRUNT_SHOTGUN				( 1 << 3)

#define HEAD_GROUP					1
#define HEAD_CGRUNT					0
...
//=========================================================
#define		FHCGRUNT_AE_RELOAD		( 2 )
#define		FHCGRUNT_AE_KICK			( 3 )
#define		FHCGRUNT_AE_BURST1		( 4 )
#define		FHCGRUNT_AE_BURST2		( 5 ) 
#define		FHCGRUNT_AE_BURST3		( 6 ) 
#define		FHCGRUNT_AE_GREN_TOSS		( 7 )
#define		FHCGRUNT_AE_GREN_LAUNCH	( 8 )
#define		FHCGRUNT_AE_GREN_DROP		( 9 )
#define		FHCGRUNT_AE_CAUGHT_ENEMY	( 10) // grunt established sight with an enemy (player only) that had previously eluded the squad.
#define		FHCGRUNT_AE_DROP_GUN		( 11) // grunt (probably dead) is dropping his mp5.


const char *CNPC_CGrunt::pGruntSentences[] =
{
...
enum
{
	FHCGRUNT_SENT_NONE = -1,
	FHCGRUNT_SENT_GREN = 0,
	FHCGRUNT_SENT_ALERT,
	FHCGRUNT_SENT_MONSTER,
	FHCGRUNT_SENT_COVER,
	FHCGRUNT_SENT_THROW,
	FHCGRUNT_SENT_CHARGE,
	FHCGRUNT_SENT_TAUNT,
} FHCGRUNT_SENTENCE_TYPES;

LINK_ENTITY_TO_CLASS(monster_human_grunt_combine, CNPC_CGrunt);
...
enum
{
	SCHED_CGRUNT_FAIL = LAST_SHARED_SCHEDULE,
	SCHED_CGRUNT_COMBAT_FAIL,
	SCHED_CGRUNT_VICTORY_DANCE,
	SCHED_CGRUNT_ESTABLISH_LINE_OF_FIRE,
	SCHED_CGRUNT_ESTABLISH_LINE_OF_FIRE_RETRY,
	SCHED_CGRUNT_FOUND_ENEMY,
	SCHED_CGRUNT_COMBAT_FACE,
	SCHED_CGRUNT_SIGNAL_SUPPRESS,
	SCHED_CGRUNT_SUPPRESS,
	SCHED_CGRUNT_WAIT_IN_COVER,
	SCHED_CGRUNT_TAKE_COVER,
	SCHED_CGRUNT_GRENADE_COVER,
	SCHED_CGRUNT_TOSS_GRENADE_COVER,
	SCHED_CGRUNT_HIDE_RELOAD,
	SCHED_CGRUNT_SWEEP,
	SCHED_CGRUNT_RANGE_ATTACK1A,
	SCHED_CGRUNT_RANGE_ATTACK1B,
	SCHED_CGRUNT_RANGE_ATTACK2,
	SCHED_CGRUNT_REPEL,
	SCHED_CGRUNT_REPEL_ATTACK,
	SCHED_CGRUNT_REPEL_LAND,
	SCHED_CGRUNT_TAKE_COVER_FAILED,
	SCHED_CGRUNT_RELOAD,
	SCHED_CGRUNT_TAKE_COVER_FROM_ENEMY,
	SCHED_CGRUNT_BARNACLE_HIT,
	SCHED_CGRUNT_BARNACLE_PULL,
	SCHED_CGRUNT_BARNACLE_CHOMP,
	SCHED_CGRUNT_BARNACLE_CHEW,
};
...
enum
{
	TASK_CGRUNT_FACE_TOSS_DIR = LAST_SHARED_TASK + 1,
	TASK_CGRUNT_SPEAK_SENTENCE,
	TASK_CGRUNT_CHECK_FIRE,
};
...
enum
{
	COND_CGRUNT_NOFIRE = LAST_SHARED_CONDITION + 1,
};
...
// Spawn
//=========================================================
void CNPC_CGrunt::Spawn()
{
	Precache();

	SetModel("models/cgrunt.mdl");
	...
	m_iSentence = FHCGRUNT_SENT_NONE;
	...
	//	m_iWeapons = FHCGRUNT_HANDGRENADE;
	...
	if (FBitSet(m_iWeapons, FHCGRUNT_SHOTGUN))
	{
...
//	else
	...
	else if (FBitSet(m_iWeapons, FHCGRUNT_GRENADELAUNCHER))
	{
	...
	SetUse( &CNPC_CGrunt::FollowerUse );
}



void CNPC_CGrunt::FollowerUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
//		}
	...
	CNPC_CGrunt::FollowerUse( pActivator, pCaller, useType, value );
}


int CNPC_CGrunt::IRelationPriority(CBaseEntity *pTarget)
{
...
//=========================================================
void CNPC_CGrunt::Precache()
{
	m_iAmmoType = GetAmmoDef()->Index("buckshot");

	PrecacheModel("models/cgrunt.mdl");//red guy
...
//=========================================================
bool CNPC_CGrunt::FOkToSpeak(void)
{
...
// started moving.
//=========================================================
void CNPC_CGrunt::SpeakSentence(void)
{
	if (m_iSentence == FHCGRUNT_SENT_NONE)
	{
		// no sentence cued up.
		return;
	}
	...
	if (FOkToSpeak())
	{
		SENTENCEG_PlayRndSz(edict(), pGruntSentences[m_iSentence], FHCGRUNT_SENTENCE_VOLUME, CGRUNT_SNDLVL, 0, m_voicePitch);
		JustSpoke();
	}
}
...
//=========================================================
//=========================================================
void CNPC_CGrunt::JustSpoke(void)
{
...
//=========================================================
void CNPC_CGrunt::PrescheduleThink(void)
{
	...
	if (m_pSquad && GetEnemy() != NULL)
	{
		...
		CNPC_CGrunt *pSquadLeader = (CNPC_CGrunt*)m_pSquad->GetLeader()->MyNPCPointer();
...
Class_T	CNPC_CGrunt::Classify(void)
{
...
//
//=========================================================
int CNPC_CGrunt::SquadRecruit(int searchRadius, int maxMembers)
{
	int squadCount;
	...
	if (m_SquadName != NULL_STRING)
	{
		...
		while (pEntity)
		{
			CNPC_CGrunt *pRecruit = (CNPC_CGrunt*)pEntity->MyNPCPointer();
...
void CNPC_CGrunt::StartNPC(void)
{
	if (!m_pSquad)
	{
		if (m_SquadName != NULL_STRING)
		{
			...
			if (GetSpawnFlags() & SF_CGRUNT_LEADER)
			{
				InitSquad();
...
// CheckMeleeAttack1
//=========================================================
int CNPC_CGrunt::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (flDist > 64)
...
//=========================================================
int CNPC_CGrunt::RangeAttack1Conditions(float flDot, float flDist)
{
...
int CNPC_CGrunt::RangeAttack2Conditions(float flDot, float flDist)
{
	m_iLastGrenadeCondition = GetGrenadeConditions(flDot, flDist);
	return m_iLastGrenadeCondition;
}

int CNPC_CGrunt::GetGrenadeConditions(float flDot, float flDist)
{
	if (!FBitSet(m_iWeapons, (FHCGRUNT_HANDGRENADE | FHCGRUNT_GRENADELAUNCHER)))
		return COND_NONE;
	...
	if (FBitSet(m_iWeapons, FHCGRUNT_HANDGRENADE))
	{
		// find feet
		if (random->RandomInt(0, 1))
		{
...
//=========================================================
bool CNPC_CGrunt::FCanCheckAttacks(void)
{
...
int CNPC_CGrunt::GetSoundInterests(void)
{
	return	SOUND_WORLD |
		SOUND_COMBAT |
		SOUND_PLAYER |
...
//=========================================================
void CNPC_CGrunt::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr)
{
	...
	if (ptr->hitgroup == 11)
	{
		...
		if (GetBodygroup(1) == HEAD_CGRUNT && (info.GetDamageType() & (DMG_BULLET | DMG_SLASH | DMG_BLAST | DMG_CLUB)))
		{
			// absorb damage
...
//=========================================================
int CNPC_CGrunt::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo)
{
...
void CNPC_CGrunt::StartFollowing(	CBaseEntity *pLeader )
{
...
//	}
	...
	CNPC_CGrunt::StartFollowing( pLeader );
}

void CNPC_CGrunt::StopFollowing( void )
{

	CNPC_CGrunt::StopFollowing();
}
...
//=========================================================
float CNPC_CGrunt::MaxYawSpeed(void)
{
	float flYS;
...
void CNPC_CGrunt::IdleSound(void)
{
	if (FOkToSpeak() && (g_fGruntQuestion || random->RandomInt(0, 1)))
	{
		if (!g_fGruntQuestion)
		{
			...
			{
			case 0: // check in
				SENTENCEG_PlayRndSz(edict(), "HG_CHECK", FHCGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				g_fGruntQuestion = 1;
				break;
			case 1: // question
				SENTENCEG_PlayRndSz(edict(), "HG_QUEST", FHCGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				g_fGruntQuestion = 2;
				break;
			case 2: // statement
				SENTENCEG_PlayRndSz(edict(), "HG_IDLE", FHCGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			}
		}
		else
		{
			...
			{
			case 1: // check in
				SENTENCEG_PlayRndSz(edict(), "HG_CLEAR", FHCGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			case 2: // question 
				SENTENCEG_PlayRndSz(edict(), "HG_ANSWER", FHCGRUNT_SENTENCE_VOLUME, SNDLVL_TALKING, 0, m_voicePitch);
				break;
			}
			g_fGruntQuestion = 0;
		}
...
bool CNPC_CGrunt::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_CGrunt::CheckAmmo(void)
{
	if (m_cAmmoLoaded <= 0)
...
//=========================================================
//=========================================================
CBaseEntity *CNPC_CGrunt::Kick(void)
{
	trace_t tr;
...
Vector CNPC_CGrunt::Weapon_ShootPosition(void)
{
	if (m_fStanding)
...
void CNPC_CGrunt::Event_Killed(const CTakeDamageInfo &info)
{
	Vector	vecGunPos;
	QAngle	vecGunAngles;
	...
	if (FBitSet(m_iWeapons, FHCGRUNT_GRENADELAUNCHER))
	{
...
//=========================================================
void CNPC_CGrunt::HandleAnimEvent(animevent_t *pEvent)
{
	Vector	vecShootDir;
	...
	case FHCGRUNT_AE_BURST1:
	{
		if (FBitSet(m_iWeapons, FHCGRUNT_9MMAR))
		{
			Shoot();
	...
	case FHCGRUNT_AE_BURST2:
			EmitSound( "Weapon_SMG1.NPC_Single");
	case FHCGRUNT_AE_BURST3:
			EmitSound( "Weapon_SMG1.NPC_Single");
		Shoot();
		break;

	case FHCGRUNT_AE_KICK:
	{
	...
	case FHCGRUNT_AE_CAUGHT_ENEMY:
	{
		if (FOkToSpeak())
		{
			SENTENCEG_PlayRndSz(edict(), "HG_ALERT", FHCGRUNT_SENTENCE_VOLUME, CGRUNT_SNDLVL, 0, m_voicePitch);
			JustSpoke();
		}
...
void CNPC_CGrunt::SetAim(const Vector &aimDir)
{
	QAngle angDir;
...
// start task
//=========================================================
void CNPC_CGrunt::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_CGRUNT_CHECK_FIRE:
		if (!NoFriendlyFire())
		{
	...
	case TASK_CGRUNT_SPEAK_SENTENCE:
		SpeakSentence();
		TaskComplete();
		break;
	...
	case TASK_CGRUNT_FACE_TOSS_DIR:
		break;
...
// RunTask
//=========================================================
void CNPC_CGrunt::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
	case TASK_CGRUNT_FACE_TOSS_DIR:
...
// PainSound
//=========================================================
void CNPC_CGrunt::PainSound(const CTakeDamageInfo &info)
{
...
// DeathSound 
//=========================================================
void CNPC_CGrunt::DeathSound(const CTakeDamageInfo &info)
{
...
// SetActivity 
//=========================================================
Activity CNPC_CGrunt::NPC_TranslateActivity(Activity eNewActivity)
{
	switch (eNewActivity)
	{
	case ACT_RANGE_ATTACK1:
	...
	case ACT_RANGE_ATTACK2:
		...
		if (m_iWeapons & FHCGRUNT_HANDGRENADE)
		{
			// get toss anim
		...
		break;
	case ACT_RUN:
		if (m_iHealth <= FHCGRUNT_LIMP_HEALTH)
		{
			// limp!
			return ACT_RUN_HURT;
		}
...
void CNPC_CGrunt::ClearAttackConditions(void)
{
...
int CNPC_CGrunt::SelectSchedule(void)
{
	// clear old sentence
	m_iSentence = FHCGRUNT_SENT_NONE;
	...
	if (GetMoveType() == MOVETYPE_FLYGRAVITY && m_NPCState != NPC_STATE_PRONE)
	{
		if (GetFlags() & FL_ONGROUND)
		{
			...
			SetMoveType(MOVETYPE_STEP);
			SetGravity(1.0);
			return SCHED_CGRUNT_REPEL_LAND;
		}
		else
		{
			// repel down a rope, 
			if (m_NPCState == NPC_STATE_COMBAT)
				return SCHED_CGRUNT_REPEL_ATTACK;
			else
				return SCHED_CGRUNT_REPEL;
		}
	}
	...
	if (HasCondition(COND_HEAR_DANGER))
	{
		...
		if (FOkToSpeak())
		{
			SENTENCEG_PlayRndSz(edict(), "HG_GREN", FHCGRUNT_SENTENCE_VOLUME, CGRUNT_SNDLVL, 0, m_voicePitch);
			JustSpoke();
		}
	...
	case NPC_STATE_PRONE:
	{
		...
		{
			return SCHED_CGRUNT_BARNACLE_CHOMP;
		}
		else
		{
			return SCHED_CGRUNT_BARNACLE_HIT;
		}
	}
	...
	case NPC_STATE_COMBAT:
	{
				...
				else
				{
					...
					if (FOkToSpeak())// && RANDOM_LONG(0,1))
					{
						if ((GetEnemy() != NULL) )
							...
							// monster
							SENTENCEG_PlayRndSz(edict(), "HG_MONST", FHCGRUNT_SENTENCE_VOLUME, CGRUNT_SNDLVL, 0, m_voicePitch);
					...
					{
						return SCHED_CGRUNT_SUPPRESS;
					}
					else
					{
						return SCHED_CGRUNT_ESTABLISH_LINE_OF_FIRE;
					}
				}
			}
		}
		...
		else if (HasCondition(COND_NO_PRIMARY_AMMO))
		{
			...
			return SCHED_CGRUNT_HIDE_RELOAD;
		}
		...
		else if (HasCondition(COND_LIGHT_DAMAGE))
		{
			...
			if (iPercent <= 90 && GetEnemy() != NULL)
			{
				...
				if (FOkToSpeak()) // && RANDOM_LONG(0,1))
				{
					SENTENCEG_PlayRndSz(edict(), "HG_COVER", FHCGRUNT_SENTENCE_VOLUME, CGRUNT_SNDLVL, 0, m_voicePitch);
					m_iSentence = FHCGRUNT_SENT_COVER;
					//JustSpoke();
				}
				return SCHED_TAKE_COVER_FROM_ENEMY;
			}
		...
		else if (FBitSet(m_iWeapons, FHCGRUNT_GRENADELAUNCHER) && HasCondition(COND_CAN_RANGE_ATTACK2) && OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2))
		{
		...
		else if (HasCondition(COND_CAN_RANGE_ATTACK1))
		{
			if (m_pSquad)
			{
				if (m_pSquad->GetLeader() != NULL)
				{
					...
					{
						return SCHED_CGRUNT_FOUND_ENEMY;
					}
				}
			}
		...
		else if (HasCondition(COND_ENEMY_OCCLUDED))
		{
			if (HasCondition(COND_CAN_RANGE_ATTACK2) && OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2))
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(edict(), "HG_THROW", FHCGRUNT_SENTENCE_VOLUME, CGRUNT_SNDLVL, 0, m_voicePitch);
					JustSpoke();
				}
				return SCHED_RANGE_ATTACK2;
			}
			else if (OccupyStrategySlotRange(SQUAD_SLOT_ENGAGE1, SQUAD_SLOT_ENGAGE2))
			{
				...
				if (FOkToSpeak())// && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "HG_CHARGE", FHCGRUNT_SENTENCE_VOLUME, CGRUNT_SNDLVL, 0, m_voicePitch);
					m_iSentence = FHCGRUNT_SENT_CHARGE;
					//JustSpoke();
				}
			...
			else
			{
				...
				{
					SENTENCEG_PlayRndSz(edict(), "HG_TAUNT", FHCGRUNT_SENTENCE_VOLUME, CGRUNT_SNDLVL, 0, m_voicePitch);
					JustSpoke();
				}
...
int CNPC_CGrunt::TranslateSchedule(int scheduleType)
{
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
	{
		if (m_pSquad)
		{
			if (g_iSkillLevel == SKILL_HARD && HasCondition(COND_CAN_RANGE_ATTACK2) && OccupyStrategySlotRange(SQUAD_SLOT_GRENADE1, SQUAD_SLOT_GRENADE2))
			{
				...
				return SCHED_CGRUNT_TOSS_GRENADE_COVER;
			}
			else
			{
				return SCHED_CGRUNT_TAKE_COVER;
			}
		}
		else
		{
			...
			{
				return SCHED_CGRUNT_GRENADE_COVER;
			}
		}
	}
	case SCHED_CGRUNT_TAKE_COVER_FAILED:
	{
	...
	case SCHED_TARGET_FACE:
		{
			...
			if ( baseType == SCHED_IDLE_STAND )
				return SCHED_FHCGRUNT_FACE_TARGET;
			else
				return baseType;
		}
		break;
	...
	case SCHED_RANGE_ATTACK1:
	{
		...
		if (m_fStanding)
			return SCHED_CGRUNT_RANGE_ATTACK1B;
		else
			return SCHED_CGRUNT_RANGE_ATTACK1A;
	}

	case SCHED_RANGE_ATTACK2:
	{
		return SCHED_CGRUNT_RANGE_ATTACK2;
	}
	case SCHED_VICTORY_DANCE:
	{
		if (m_pSquad)
		{
			if (!m_pSquad->IsLeader(this))
			{
				return SCHED_CGRUNT_FAIL;
			}
		}

		return SCHED_CGRUNT_VICTORY_DANCE;
	}
	case SCHED_CGRUNT_SUPPRESS:
	{
		if (GetEnemy()->IsPlayer() && m_fFirstEncounter)
		{
			m_fFirstEncounter = FALSE;// after first encounter, leader won't issue handsigns anymore when he has a new enemy
			return SCHED_CGRUNT_SIGNAL_SUPPRESS;
		}
		else
		{
			return SCHED_CGRUNT_SUPPRESS;
	...
	case SCHED_FAIL:
	{
		if (GetEnemy() != NULL)
		{
			...
			return SCHED_CGRUNT_COMBAT_FAIL;
		}

		return SCHED_CGRUNT_FAIL;
	}
	case SCHED_CGRUNT_REPEL:
	{
	...
	case SCHED_CGRUNT_REPEL_ATTACK:
	{
		Vector vecVel = GetAbsVelocity();
		if (vecVel.z > -128)
		{
...
class CNPC_CGruntRepel :public CAI_BaseNPC
{
	DECLARE_CLASS(CNPC_CGruntRepel, CAI_BaseNPC);
public:
	void Spawn(void);
	void Precache(void);
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CNPC_CGruntRepel)
DEFINE_USEFUNC(RepelUse),
//DEFINE_FIELD( m_iSpriteTexture, FIELD_INTEGER ),
END_DATADESC()

void CNPC_CGruntRepel::Spawn(void)
{
	Precache();
	SetSolid(SOLID_NONE);

	SetUse(&CNPC_CGruntRepel::RepelUse);
}

void CNPC_CGruntRepel::Precache(void)
{
	UTIL_PrecacheOther("monster_human_grunt");
	m_iSpriteTexture = PrecacheModel("sprites/rope.vmt");
}

void CNPC_CGruntRepel::RepelUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	trace_t tr;
...
//
//------------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC(monster_human_grunt, CNPC_CGrunt)
...
DECLARE_CONDITION(COND_CGRUNT_NOFIRE)

DECLARE_TASK(TASK_CGRUNT_FACE_TOSS_DIR)
DECLARE_TASK(TASK_CGRUNT_SPEAK_SENTENCE)
DECLARE_TASK(TASK_CGRUNT_CHECK_FIRE)
...
"	Tasks"
"		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_CGRUNT_ESTABLISH_LINE_OF_FIRE_RETRY"
"		TASK_GET_PATH_TO_ENEMY		0"
"		TASK_CGRUNT_SPEAK_SENTENCE	0"
"		TASK_RUN_PATH				0"
"		TASK_WAIT_FOR_MOVEMENT		0"
"	"
"	Interrupts"
...
"	Tasks"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CGRUNT_TAKE_COVER_FROM_ENEMY"
...
"		TASK_FACE_ENEMY			0"
"		TASK_WAIT				1.5"
"		TASK_SET_SCHEDULE		SCHEDULE:SCHED_CGRUNT_SWEEP"
"	"
"	Interrupts"
"		COND_NEW_ENEMY"
"		COND_ENEMY_DEAD"
...
"	Tasks"
"		TASK_STOP_MOVING			0"
"		TASK_SET_FAIL_SCHEDULE		SCHEDULE:SCHED_CGRUNT_TAKE_COVER_FAILED"
"		TASK_WAIT					0.2"
...
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_CGRUNT_WAIT_IN_COVER"
...
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_CGRUNT_TAKE_COVER_FROM_ENEMY"
"	"
"	Interrupts"
)
...
"	Tasks"
"		TASK_STOP_MOVING				0"
"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_CGRUNT_RELOAD"
"		TASK_FIND_COVER_FROM_ENEMY		0"
"		TASK_RUN_PATH					0"
"		TASK_WAIT_FOR_MOVEMENT			0"
"		TASK_REMEMBER					MEMORY:INCOVER"
...
"	Tasks"
"		TASK_STOP_MOVING			0"
"		TASK_CGRUNT_FACE_TOSS_DIR	0"
"		TASK_PLAY_SEQUENCE			ACTIVITY:ACT_RANGE_ATTACK2"
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_CGRUNT_WAIT_IN_COVER" // don't run immediately after throwing grenade.
"	"
"	Interrupts"
)
...
//		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_BARNEY_STOP_FOLLOWING"
	...
	DEFINE_SCHEDULE
	(
		SCHED_FHCGRUNT_FACE_TARGET,
	
		"	Tasks"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_FACE_TARGET			0"
		"		TASK_SET_ACTIVITY			ACTIVITY:ACT_IDLE"
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_FHCGRUNT_FOLLOW"
		"	"
		"	Interrupts"
		"		COND_GIVE_WAY"
		"		COND_NEW_ENEMY"
...
"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_CGRUNT_BARNACLE_PULL"
""
"	Interrupts"
)
...
"		 TASK_SET_SCHEDULE			SCHEDULE:SCHED_CGRUNT_BARNACLE_CHEW"
""
"	Interrupts"
)
