...
void CWeaponMP5::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	...
	case EVENT_WEAPON_SMG1:
		...
		pGrenade->SetThrower( pOperator );
		pGrenade->SetDamage(sk_npc_dmg_smg1_grenade.GetFloat());

		// FIXME: arrgg ,this is hard coded into the weapon???
		m_flNextGrenadeCheck = gpGlobals->curtime + 3;// wait six seconds before even looking again to see if a grenade can be thrown.
		}*/
		{
		WeaponSound( WPN_DOUBLE );
				Vector vecTarget;
			CAI_BaseNPC *npc = pOperator->MyNPCPointer();
			CBaseEntity *pEnemy = pOperator->GetEnemy();
			if (!npc)
				return;
			...
			else
			{
				...
				float time = vecThrow.Length() / 1500.0;
				vecThrow = vecThrow * (1.0 / time);

				// adjust upward toss to compensate for gravity loss
				vecThrow.z += (GetGravity() * 0.5) * time * 0.5;
			}
			...
			pGrenade->SetGravity(0.15); // lower gravity since grenade is aerodynamic and engine doesn't know it.
