...
BEGIN_SIMPLE_DATADESC( CRollerControllerBeta )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
IMotionEvent::simresult_e CRollerControllerBeta::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	if( m_fIsStopped )
	{
		return SIM_NOTHING;
...
BEGIN_DATADESC( CNPC_Roller )
	...
	DEFINE_FIELD( m_flTimeMarcoSound, FIELD_TIME ),
	DEFINE_FIELD( m_flTimePoloSound, FIELD_TIME ),
	DEFINE_FIELD( m_fHACKJustSpawned, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_vecUnstickDirection, FIELD_VECTOR ),
	DEFINE_FIELD( m_flLastZPos, FIELD_FLOAT ),
	DEFINE_FIELD( m_iFail, FIELD_INTEGER ),
	DEFINE_FIELD( m_flForwardSpeed, FIELD_FLOAT ),
	DEFINE_ARRAY( m_iAccessCode, FIELD_STRING,  ROLLER_CODE_DIGITS  ),
	DEFINE_FIELD( m_iCodeProgress, FIELD_INTEGER ),
...
void CNPC_Roller::Precache( void )
{
	...
	PrecacheScriptSound("NPC_Roller.PowerOn");
	PrecacheScriptSound("NPC_Roller.PowerOff");
	PrecacheScriptSound("NPC_Roller.Polo");
	PrecacheScriptSound("NPC_Roller.Marco");
...
void CNPC_Roller::Spawn( void )
{
	...
	m_pMotionController->AttachObject( pPhysicsObject, false );
...
{
	return(pHint->HintType() == HINT_NOT_USED_ROLLER_PATROL_POINT);
}
...
void CNPC_Roller::StartTask( const Task_t *pTask )
{
	...
	case TASK_ROLLER_FIND_PATROL_NODE:
		...
		SetHintNode( CAI_HintManager::FindHint( this, HINT_NOT_USED_ROLLER_PATROL_POINT, 0, 1024 )); 
	...
	case TASK_ROLLER_ISSUE_CODE:
		if( gpGlobals->curtime >= m_flWaitFinished )
		{
			...
			else
			{
				...
				params.m_pSoundName = pCodeSounds[m_iAccessCode[m_iCodeProgress]];
